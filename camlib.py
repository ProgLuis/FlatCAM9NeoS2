...
# Rasterio es opcional y NO se instala en este entorno
try:
    import rasterio
    ...
    def shapes(*args, **kwargs):
        raise RuntimeError("Funciones raster no disponibles: falta rasterio")
...
def is_iterable_geo(obj):
    return isinstance(obj, MULTI_GEOMS)
...
class Geometry(object):
    ...
    def plot_temp_shapes(self, element, color='red'):
        ...
        if hasattr(element, "geoms"):
            for sub_el in element.geoms:
                self.plot_temp_shapes(sub_el, color=color)
            return
    ...
    def find_polygon(self, point, geoset=None):
        """
        Find a polygon/geometry that contains the given point.
        Compatible with Shapely 2.x.
        """
        ...
        for sub_geo in iter_geom(geoset):
            if sub_geo is None:
                continue

            if isinstance(sub_geo, LinearRing):
                sub_geo = Polygon(sub_geo)

            if isinstance(sub_geo, (list, tuple)) or hasattr(sub_geo, "geoms"):
                found_geo = self.find_polygon(point, geoset=sub_geo)
                if found_geo is not None:
                    return found_geo
                continue

            try:
                if sub_geo.contains(test_point):
                    return sub_geo
    ...
    def flatten(self, geometry=None, reset=True, pathonly=False):
        ...
        Polygons are expanded into exterior and interiors if pathonly=True.
        Compatible with Shapely 2.x.
        """
    ...
    def isolation_geometry(self, offset, geometry=None, iso_type=2, corner=None, follow=None, passes=0,
        ...
        try:
            for pol in iter_geom(working_geo):
                if self.app.abort_flag:
                    # graceful abort requested by the user
                    raise grace
                if offset == 0:
    ...
    def import_svg(self, filename, object_type=None, flip=True, units=None):
        ...
        if viewbox is not None:
            try:
                ...
                if len(viewbox_values) == 4:
                    vb_minx = viewbox_values[0] * factor
                    vb_miny = viewbox_values[1] * factor
                    vb_maxx = (viewbox_values[0] + viewbox_values[2]) * factor
                    vb_maxy = (viewbox_values[1] + viewbox_values[3]) * factor
                    viewbox_bounds = (vb_minx, vb_miny, vb_maxx, vb_maxy)
            except Exception as e:
                log.debug("camlib.Geometry.import_svg() viewBox bounds --> %s" % str(e))
        ...
        for poly in geos_polys:
            ...
            if not poly.is_valid:
                try:
                    ...
                    log.debug("camlib.Geometry.import_svg() poly buffer(0) --> %s" % str(e))
                    continue

            if poly is not None and not poly.is_empty:
                if viewbox_bounds is not None:
                    poly_bounds = poly.bounds
                    if all(abs(poly_bounds[i] - viewbox_bounds[i]) <= viewbox_tol for i in range(4)):
                        continue
        ...
        if clean_polys:
            try:
                ...
                if not merged_polys.is_valid:
                    merged_polys = merged_polys.buffer(0)
                geos = [merged_polys]
            except Exception as e:
                log.debug("camlib.Geometry.import_svg() unary_union --> %s" % str(e))
                geos = clean_polys
        else:
            geos = []

        for l in iter_geom(merged_lines):
    ...
    def import_image(self, filename, flip=True, units='MM', dpi=96, mode='black', mask=None):
        ...
        if HAVE_RASTERIO:
            ...
            # Opcional: solo aviso o error suave
            # log.warning("Función raster no disponible: falta rasterio")
            return
    ...
    def clear_polygon2(self, polygon_to_clear, tooldia, steps_per_circle, seedpoint=None, overlap=0.15,
        ...
        Shapely 2.x friendly:
        - Does not iterate directly over MultiLineString/MultiPolygon/GeometryCollection.
        - Uses iter_geom() for multipart geometries.
        """
        ...
        if path_margin is None or path_margin.is_empty:
            return None
        ...
        while True:
            ...
            # Provide the app with a way to process GUI events when in a blocking loop.
            QtWidgets.QApplication.processEvents()
            ...
            # path can be LineString, MultiLineString or GeometryCollection.
            for p in iter_geom(path):
        ...
        if contour:
            buffered_poly = polygon_to_clear.buffer(-tooldia / 2, int(steps_per_circle))

            outer_edges = []
            inner_edges = []

            for x in iter_geom(buffered_poly):
                if not isinstance(x, Polygon) or x.is_empty:
                    continue

                outer_edges.append(x.exterior)
    ...
    def clear_polygon3(self, polygon, tooldia, steps_per_circle, overlap=0.15, connect=True, contour=True,
        ...
        This algorithm draws horizontal or vertical lines inside the polygon.

        Shapely 2.x friendly:
        - Does not iterate directly over MultiLineString/MultiPolygon/GeometryCollection.
        - Uses iter_geom() for multipart geometries.
        - Inserts only line-like geometry into FlatCAMRTreeStorage.
        """
        ...
        def add_line_parts(geo_obj):
            """
            Add only valid line-like parts to lines_trimmed.
            This avoids direct iteration over Shapely multipart geometries.
            """
            for part in iter_geom(geo_obj):
                if part is None or part.is_empty:
                    continue

                if isinstance(part, (LineString, LinearRing)):
                    lines_trimmed.append(part)
                    if prog_plot:
                        self.plot_temp_shapes(part)
                else:
                    log.debug("camlib.Geometry.clear_polygon3(). Not a line: %s" % str(type(part)))
        ...
        if margin_poly is None or margin_poly.is_empty:
            return None
        ...
        for line in iter_geom(lines_trimmed):
        ...
        if contour:
            for poly in iter_geom(margin_poly):
    ...
    def fill_with_lines(self, line, aperture_size, tooldia, steps_per_circle, overlap=0.15, connect=True, contour=True,
        ...
        This algorithm draws parallel lines inside the polygon generated
        by buffering the original line.
        ...
        if not isinstance(line, LineString):
            log.debug("camlib.Geometry.fill_with_lines() --> Not a LineString but %s" % str(type(line)))
            return None
        ...
        def add_line_parts(geo_obj):
            """
            Add only valid line-like parts to lines_trimmed.
            Avoids direct iteration over Shapely multipart geometries.
            """
            for part in iter_geom(geo_obj):
                if part is None or part.is_empty:
                    continue
                ...
                if isinstance(part, (LineString, LinearRing)):
                    ...
                    log.debug("camlib.Geometry.fill_with_lines(). Not a line: %s" % str(type(part)))
        ...
        # Add lines to storage
        for line_part in iter_geom(lines_trimmed):
            if line_part is None or line_part.is_empty:
                continue

            if isinstance(line_part, (LineString, LinearRing)):
                geoms.insert(line_part)
            else:
                log.debug("camlib.Geometry.fill_with_lines(). Not a line: %s" % str(type(line_part)))
    ...
    def rotate(self, angle, point):
        ...
        def rotate_geom(obj):
            ...
            if isinstance(obj, (list, tuple)) or hasattr(obj, "geoms"):
                new_obj = []
                for g in iter_geom(obj):
    ...
    * ``gcode_parsed`` (list): Each is a dictionary:
    ...
    def generate_from_multitool_geometry(self, geometry, append=True, tooldia=None, offset=0.0, tolerance=0, z_cut=1.0,
        ...
        if offset != 0.0:
            ...
            temp_solid_geometry = list(iter_geom(geometry))            
