...
from shapely.geometry import Point, MultiPoint, Polygon, LineString, LinearRing, box
...
class CNCJobObject(FlatCAMObj, CNCjob):
    ...
    def plot_probing_geo(self, geometry, visibility, custom_color=None):
        if visibility:
            ...
            try:
                ...
                for sh in iter_geom_recursive(geometry):
    ...
    def generate_voronoi_geometry(self, pts):
        ...
        for p in iter_geom_recursive(voronoi_union):
            inter_geo = p.intersection(env)
            if inter_geo is not None and not inter_geo.is_empty:
                new_voronoi.append(inter_geo)
