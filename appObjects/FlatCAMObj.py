...
class FlatCAMObj(QtCore.QObject):
    ...
    def iter_geom(geometry):
        ...
        if isinstance(geometry, (list, tuple)):
            geo_list = []
            for geo in geometry:
                geo_list += FlatCAMObj.iter_geom(geo)
            return geo_list
        ...
        def job_thread(obj_prop):
            ...
            if geo:
                ...
                if obj_prop.kind.lower() == 'gerber' and geo:
                    ...
                    for geo_el in self.iter_geom(geo):
                        try:
                    ...
                    try:
                        if obj_prop.tools:
                            for tool_k in obj_prop.tools:
                                ...
                                try:                                    
                                    x0, y0, x1, y1 = unary_union(self.iter_geom(t_geo)).bounds
                                except Exception:
                                    continue
                                xmin.append(x0)
                                ymin.append(y0)
                ...
                if obj_prop.kind.lower() == 'gerber' and obj_prop.tools:
                    ...
                    for tool_k in obj_prop.tools:
                        if 'solid_geometry' in obj_prop.tools[tool_k]:
                            geo_tools += self.iter_geom(obj_prop.tools[tool_k]['solid_geometry'])  
            ...
            if obj_prop.kind.lower() != 'cncjob':
                ...
                if geo:
                    ...
                    if geo_list:
                        try:
                            env_obj = unary_union(geo_list).convex_hull
                            area_chull = env_obj.area
                        except Exception as er:
                            area_chull = 0
                            log.debug("FlatCAMObj.add_properties_items() geo convex hull --> %s" % str(er))
                    else:
                        area_chull = 0
                    ...
                    try:
                        ...
                        if obj_prop.tools:
                            ...
                            for tool_k in obj_prop.tools:
                                area_el = unary_union(
                                    self.iter_geom(obj_prop.tools[tool_k]['solid_geometry'])
                                ).convex_hull
