...
def iter_geom(obj):
    """
    Iterate safely over simple, list-like and multipart Shapely geometries.
...
def iter_point_parts(obj):
    """
    Yield only valid Point parts.
    """
    for geo in iter_geom_recursive(obj):
        if isinstance(geo, Point) and not geo.is_empty:
            yield geo
...
class GerberObject(FlatCAMObj, Gerber):
    ...
    def generate_envelope(self, offset, invert, geometry=None, env_iso_type=2, follow=None, nr_passes=0):
        ...
        if invert:
            try:
                ...
                for p in iter_geom_recursive(geom):
                ...
                if not pl:
                    log.debug("GerberObject.isolate().generate_envelope() Error --> Empty geometry after invert.")
                    return 'fail'

                polygon_parts = [g for g in pl if isinstance(g, Polygon)]
                line_parts = [g for g in pl if isinstance(g, LineString)]

                if polygon_parts and not line_parts:
                    geom = MultiPolygon(polygon_parts)
                elif len(pl) == 1:
                    geom = pl[0]
                else:
                    geom = pl
    ...
    def plot(self, kind=None, **kwargs):
        ...
        # Shapely 2.x friendly geometry iterator.
        geometry = list(iter_geom_recursive(geometry))
                ...
                for g in geometry:
                    ...
                    if isinstance(g, (Polygon, LineString, LinearRing)):
                        self.add_shape(
                            shape=g,
                            color=color,
                            face_color=random_color() if self.options['multicolored'] else face_color,
                            visible=visible
                        )
            else:
                for g in geometry:
                    ...
                    if aperture_to_plot_mark in self.apertures:
                        for elem in app_obj.apertures[aperture_to_plot_mark]['geometry']:
                            if 'solid' in elem:
                                ...
                                if only_flashes and not isinstance(elem.get('follow'), Point):
                                    continue
                                
                                geo = elem['solid']

                                for el in iter_geom_recursive(geo):
    ...
    def merge(grb_list, grb_final):
        ...
        for grb in grb_list:
            ...
            if type(grb) is list:
                ...
                for geos in iter_geom_recursive(grb.solid_geometry):
        ...
        solid_polys = list(iter_polygon_parts(grb_final.solid_geometry))
        follow_polys = list(iter_polygon_parts(grb_final.follow_geometry))

        if solid_polys:
            grb_final.solid_geometry = MultiPolygon(solid_polys)
        else:
            grb_final.solid_geometry = grb_final.solid_geometry

        if follow_polys:
            grb_final.follow_geometry = MultiPolygon(follow_polys)
        else:
            grb_final.follow_geometry = grb_final.follow_geometry
