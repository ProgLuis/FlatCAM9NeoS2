...
def iter_geom_recursive(obj):
    """
    Recursively yield non-empty simple geometries from nested lists,
...
def iter_polygon_parts(obj):
    """
    Yield only valid Polygon parts.
    """
    for geo in iter_geom_recursive(obj):
        if isinstance(geo, Polygon) and not geo.is_empty:
            yield geo


def iter_line_parts(obj):
    """
    Yield only valid LineString/LinearRing parts.
    """
    for geo in iter_geom_recursive(obj):
...
class GeometryObject(FlatCAMObj, Geometry):
    ...
    def export_dxf(self):
        ...
        try:
            ...
            def g2dxf(dxf_space, geo_obj):
                for poly in iter_polygon_parts(geo_obj):
                ...
                for line in iter_line_parts(geo_obj):
            ...
            if self.multigeo:
                for tool in self.tools:
                    for geo in iter_geom_recursive(self.tools[tool]['solid_geometry']):
                        multigeo_solid_geometry.append(geo)
            else:
                multigeo_solid_geometry = self.solid_geometry

            for geo in iter_geom_recursive(multigeo_solid_geometry):
                g2dxf(msp, geo)

                # points = GeometryObject.get_pts(geo)
                # msp.add_lwpolyline(points)
        except Exception as e:
            log.debug("GeometryObject.export_dxf() --> %s" % str(e))
    ...
    def scale(self, xfactor, yfactor=None, point=None):
        ...
        def scale_recursion(geom):
            ...
            if isinstance(geom, list):
                return [scale_recursion(local_geom) for local_geom in geom]

            if isinstance(geom, tuple):
                return tuple(scale_recursion(local_geom) for local_geom in geom)
    ...
    def offset(self, vect):
        ...
        def translate_recursion(geom):
            ...
            if isinstance(geom, list):
                return [translate_recursion(local_geom) for local_geom in geom]

            if isinstance(geom, tuple):
                return tuple(translate_recursion(local_geom) for local_geom in geom)
    ...
    def plot_element(self, element, color=None, visible=None):
        ...
        visible = self.options['plot'] if visible is None else visible

        for sub_el in iter_geom_recursive(element):
            self.add_shape(
                shape=sub_el,
                color=color,
                visible=visible,
    ...
    def merge(geo_list, geo_final, multi_geo=None, fuse_tools=None):
        ...
        geo_final.solid_geometry = list(iter_geom_recursive(geo_final.solid_geometry))
        ...
        for geo_obj in geo_list:
            ...
            # If not list, just append
                ...
                try:
                    for geo in iter_geom_recursive(geo_obj.solid_geometry):
                        new_solid_geometry.append(deepcopy(geo))
                except Exception as e:
        ...
        if new_tools and fuse_tools is True:
            ...
            for i_lst in intersect_list:
                ...
                for old_tool in i_lst:
                    for geo in iter_geom_recursive(new_tools[old_tool]['solid_geometry']):
                        new_solid_geo.append(deepcopy(geo))
    ...
    def get_pts(o):
        ...
        for geo in iter_geom_recursive(o):
            if isinstance(geo, Polygon):
                pts += list(geo.exterior.coords)
