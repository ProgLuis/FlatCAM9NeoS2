...
from shapely.geometry import LineString, LinearRing, MultiLineString, Point, Polygon, MultiPolygon, GeometryCollection, box
...
from camlib import distance, arc, three_point_circle, iter_geom
...
def iter_polygon_parts(geo):
    """
    Devuelve solo partes Polygon válidas de una geometría simple o multipart.
    ...
    if isinstance(geo, (list, tuple)):
        for item in geo:
            for sub_item in iter_polygon_parts(item):
                yield sub_item
        return

    if hasattr(geo, "geoms"):
        for item in geo.geoms:
            for sub_item in iter_polygon_parts(item):
                yield sub_item
        return


def first_polygon_part(geo):
    """
    Devuelve el primer Polygon válido dentro de una geometría.
    """
    for poly in iter_polygon_parts(geo):
        return poly
    return None
...
class DrawToolShape(object):
    ...
    def get_pts(o):
        """
        Returns a list of all points in the object.
        Compatible with Shapely 1.x and Shapely 2.x.
        """
        pts = []
        ...
        if isinstance(o, dict):
            for key in ('solid', 'follow', 'clear'):
                if key in o and o[key] is not None:
                    pts += DrawToolShape.get_pts(o[key])
            return pts

        if isinstance(o, (list, tuple)):
            if len(o) == 2 and all(isinstance(c, (int, float)) for c in o):
                pts.append((o[0], o[1]))
                return pts
        ...
        if hasattr(o, "geoms"):
            for geom in o.geoms:
                pts += DrawToolShape.get_pts(geom)
            return pts

        if isinstance(o, Polygon):
            pts += DrawToolShape.get_pts(o.exterior)
            for ring in o.interiors:
                pts += DrawToolShape.get_pts(ring)
            return pts

        if isinstance(o, (LineString, LinearRing)):
            geom_to_use = o
            if DrawToolShape.tolerance is not None:
                geom_to_use = geom_to_use.simplify(DrawToolShape.tolerance)
            pts += list(geom_to_use.coords)
            return pts
...
class PadEditorGrb(ShapeToolEditorGrb):
    ...
    def util_shape(self, point):
        ...
        # Safety guard: avoid creating Shapely geometry with None coordinates
...
class PoligonizeEditorGrb(ShapeToolEditorGrb):
    ...
    def make(self):
        ...
        for geo_shape in self.draw_app.selected:
            ...
            if 'solid' not in geometric_data:
                continue

            for poly in iter_polygon_parts(geometric_data['solid']):
                exterior_geo.append(Polygon(poly.exterior))

        if not exterior_geo:
            self.draw_app.in_action = False
            self.complete = True
            self.draw_app.app.inform.emit('[ERROR_NOTCL] %s' % _("Failed. No valid polygon geometry."))
            self.draw_app.select_tool("select")
            return
        ...
        for geo in iter_polygon_parts(fused_geo):
            geo = geo.buffer(0)
            ...
            self.draw_app.on_grb_shape_complete(
                current_storage,
                specific_shape=DrawToolShape(deepcopy(new_el))
            )
        ...
        for sel_dia in self.selected_apertures:
            ...
            for select_shape in self.draw_app.get_selected():
                if select_shape in self.current_storage:
                    ...
                    # Evita redibujar por cada shape copiado.
                    self.draw_app.on_grb_shape_complete(
            ...
            for shp in sel_shapes_to_be_deleted:
                if shp in self.draw_app.selected:
                    self.draw_app.selected.remove(shp)
...
class EraserEditorGrb(ShapeToolEditorGrb):
    ...
    def make(self):
        ...
        utility_geo = self.utility_geometry(data=self.destination)

        if utility_geo is None or utility_geo.geo is None:
            self.draw_app.app.inform.emit('[WARNING_NOTCL] %s' % _("Failed. Nothing selected."))
            return

        for eraser_shape in utility_geo.geo:
            if 'solid' not in eraser_shape:
                continue

            temp_shape = eraser_shape['solid'].buffer(0.0000001)

            for poly in iter_polygon_parts(temp_shape):
                eraser_sel_shapes.append(Polygon(poly.exterior))

        if not eraser_sel_shapes:
            self.draw_app.app.inform.emit('[WARNING_NOTCL] %s' % _("Failed. No valid erase geometry."))
            return
...
class AppGerberEditor(QtCore.QObject):
    ...
    def disconnect_canvas_event_handlers(self):
        ...
        try:
            ...
            pass
        try:
            self.app.ui.grb_draw_semidisc.triggered.disconnect(self.on_add_semidisc)
        except (TypeError, AttributeError):
            pass
        try:
            ...
            def worker_job(app_obj):
                with app_obj.app.proc_container.new('%s ...' % _("Loading")):
                    ...
                    if global_clear_geo:
                        ...
                        global_clear_geo = list(iter_geom(global_clear_geo))                        
    ...
    def new_edited_gerber(self, outname, aperture_storage):
        ...
        def obj_init(grb_obj, app_obj):
                    ...
                    if k == 'geometry':
                        ...
                        for geo_el in val:
                            ...
                            if 'follow' in geometric_data:
                                ...
                                if 'solid' in geometric_data:
                                    solid_parts = list(iter_polygon_parts(geometric_data['solid']))

                                    if not solid_parts:
                                        continue

                                    if len(solid_parts) == 1:
                                        geo_f = solid_parts[0].exterior
                                    else:
                                        geo_f = MultiLineString([
                                            LineString(poly.exterior.coords)
                                            for poly in solid_parts
            ...
            for poly in poly_buffer:
                for sub_poly in iter_geom(poly):
                    if isinstance(sub_poly, Polygon) and not sub_poly.is_empty:
                        clean_poly_buffer.append(sub_poly)

            if clean_poly_buffer:
                new_poly = MultiPolygon(clean_poly_buffer)            
                new_poly = new_poly.buffer(0.00000001)
                new_poly = new_poly.buffer(-0.00000001)
                new_poly = list(iter_geom(new_poly))
            else:
                new_poly = []
    ...
    def on_grb_shape_complete(self, storage=None, specific_shape=None, no_plot=False):
        ...
        # Safety guard: avoid adding empty geometry
        if geo is None:
            log.debug("AppGerberEditor.on_grb_shape_complete() --> Empty geometry skipped: geo is None.")
            return

        if hasattr(geo, "geo") and geo.geo is None:
            log.debug("AppGerberEditor.on_grb_shape_complete() --> Empty geometry skipped: geo.geo is None.")
            return

        if hasattr(geo, "geo") and hasattr(geo.geo, "is_empty") and geo.geo.is_empty:
            log.debug("AppGerberEditor.on_grb_shape_complete() --> Empty geometry skipped: geo.geo is empty.")
            return
        ...
        self.tool_shape.clear(update=False)
    ...
    def add_gerber_shape(self, shape_element, storage):
        ...
        if not isinstance(shape_element, DrawToolShape):
            log.debug("AppGerberEditor.add_gerber_shape() --> Expected DrawToolShape, got %s" % str(type(shape_element)))
            return

        if shape_element.geo is None:
            log.debug("AppGerberEditor.add_gerber_shape() --> Empty geometry skipped: geo is None.")
            return

        if isinstance(shape_element.geo, list) and len(shape_element.geo) == 0:
            log.debug("AppGerberEditor.add_gerber_shape() --> Empty geometry skipped: geo is [].")
            return

        if hasattr(shape_element.geo, "is_empty") and shape_element.geo.is_empty:
            log.debug("AppGerberEditor.add_gerber_shape() --> Empty Shapely geometry skipped.")
            return
    ...
    def on_canvas_move(self, event):
        ...
        if self.app.grid_status():
            ...
            self.app.app_cursor.set_data(np.asarray([(x, y)]), edge_color=self.app.cursor_color_3D,
    ...
    def draw_utility_geometry(self, geo_shape):
        # it's a DrawToolShape therefore it stores its geometry in the geo attribute
        geometry = geo_shape.geo

        if geometry is None:
            return

        geo_items = geometry if isinstance(geometry, (list, tuple)) else [geometry]
    ...
    def plot_all(self):
        ...
        Optimized for fewer redraw/update calls.
        """
        ...
        with self.app.proc_container.new('%s ...' % _("Plotting")):
            ...
            for storage_data in self.storage_dict.values():
                if 'geometry' not in storage_data:
                    continue

                for elem in storage_data['geometry']:
                    geometric_data = elem.geo.get('solid', None)

                    if geometric_data is None:
                        continue

                    if elem in selected_set:
                        self.plot_shape(
    ...
    def on_buffer(self):
        ...
        def buffer_recursion(geom_el, selection):
            if isinstance(geom_el, (list, tuple)):
                geoms = []
                ...
                for geo_el in self.storage_dict[apcode]['geometry']:
                    if 'solid' in geo_el.geo:
                        ...
                        if float(upper_threshold_val) > area > float(lower_threshold_val):
                            poly_ref = first_polygon_part(geo_el.geo['solid'])
                            if poly_ref is None:
                                continue

                            current_pos = poly_ref.exterior.coords[-1]
...
class TransformEditorTool(AppTool):
    ...
    def on_rotate_action(self, val, point):
        ...
        self.app.inform.emit('%s...' % _("Applying Rotate"))

        with self.app.proc_container.new(_("Applying Rotate")):
            try:
                ...
                self.app.inform.emit('%s...' % _("Plotting"))
                self.draw_app.plot_all()
