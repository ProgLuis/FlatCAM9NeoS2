...
from camlib import distance, arc, three_point_circle, Geometry, FlatCAMRTreeStorage, iter_geom
...
class FCMove(FCShapeTool):
    ...
    def selection_bbox(self):
        ...
        for select_shape in self.draw_app.get_selected():
            ...
            if hasattr(geometric_data, "geoms"):
                for g in iter_geom(geometric_data):
                    if g is not None and not g.is_empty:
                        geo_list.append(g)
            else:
                if not geometric_data.is_empty:
...
class FCEraser(FCShapeTool):
    ...
    def make(self):
        ...
        eraser_geo_items = utility_geo.geo if isinstance(utility_geo.geo, (list, tuple)) else [utility_geo.geo]

        for geo_item in eraser_geo_items:
            if geo_item is None:
                continue

            for eraser_shape in iter_geom(geo_item):
                if eraser_shape is None or eraser_shape.is_empty:
                    continue

                temp_shape = eraser_shape.buffer(0.0000001)

                if isinstance(temp_shape, Polygon) and not temp_shape.is_empty:
        ...
        for obj_shape in self.storage.get_objects():
            try:
                ...
                if geometric_data is not None and eraser_sel_shapes.intersects(geometric_data):
...
class AppGeoEditor(QtCore.QObject):
    ...
    def draw_utility_geometry(self, geo):
        ...
        try:
            ...
            geo_items = geo.geo if isinstance(geo.geo, (list, tuple)) else [geo.geo]

            for el in geo_items:
                if el is None:
                    continue

                if isinstance(el, (MultiPolygon, MultiLineString)):
                    for sub_geo in iter_geom(el):
                        self.tool_shape.add(
                            shape=sub_geo,
    ...
    def plot_shape(self, geometry=None, color='#000000FF', linewidth=1):
        ...
        if isinstance(geometry, Polygon):
            ...
            plot_elements += self.plot_shape(
                geometry=list(geometry.interiors),
                color=color,
                linewidth=linewidth
            )
            return plot_elements
    ...
    def cutpath(self):
        ...
        if type(target.geo) == Polygon:
            ...
            try:
                for linestring in iter_geom(target.geo):
    ...
    def paint(self, tooldia, overlap, margin, connect, contour, method):
        ...
        def recurse(geometry, reset=True):
            ...
            if hasattr(geometry, "geoms"):
                for geo_el in iter_geom(geometry):
                    if geo_el is not None and not geo_el.is_empty:
                        recurse(geometry=geo_el, reset=False)
                return self.flat_geo

            if isinstance(geometry, LinearRing):
                self.flat_geo.append(Polygon(geometry))
        ...
        if not results:
            self.app.inform.emit('[WARNING_NOTCL] %s' % _("Paint produced no geometry."))
            return
    ...
    def flatten(self, geometry, orient_val=1, reset=True, pathonly=False):
        ...
        if isinstance(geometry, (list, tuple)):
            for geo in geometry:
                self.flatten(geometry=geo, orient_val=orient_val, reset=False, pathonly=pathonly)
            return self.flat_geo
        ...
        if pathonly and isinstance(geometry, Polygon):
            ...
            self.flatten(geometry=list(geometry.interiors), reset=False, pathonly=True)
        else:
            self.flat_geo.append(geometry)
