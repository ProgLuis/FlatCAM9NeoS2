...
from shapely.geometry import LineString, LinearRing, MultiLineString, Polygon, MultiPolygon, Point, GeometryCollection
...
def iter_line_parts(obj):
    """
    Devuelve solo partes LineString/LinearRing válidas.
    ...
    for geo in iter_geom(obj):
        ...
        if hasattr(geo, "geoms"):
            for sub_geo in iter_line_parts(geo):
                yield sub_geo


def longest_line_part(obj):
    """
    Devuelve la parte lineal más larga dentro de una geometría.
    Útil cuando intersection() devuelve MultiLineString o GeometryCollection.
    """
    line_parts = list(iter_line_parts(obj))
    ...
    def click_release(self, pos):
        ...
        try:
            ...
            if isinstance(closest_shape.geo, MultiLineString):
                first_geo = next(iter(closest_shape.geo.geoms))
                x_coord, y_coord = first_geo.xy
                ...
                for select_shape in self.draw_app.get_selected():
                    if select_shape in self.current_storage.get_objects():
                        ...
                        if isinstance(select_shape.geo, MultiLineString):
                            ...
                            cut_line_with_max_length = longest_line_part(cut_line_with_max_length)

                            if cut_line_with_max_length is None:
                            ...
                            if len(cut_line_with_max_length_coords) < 2:
                                self.draw_app.app.inform.emit('[ERROR_NOTCL] %s' % _("Cancelled."))
                                return
...
class MoveEditorExc(FCShapeTool):
    ...
    def selection_bbox(self):
        ...
        for select_shape in self.draw_app.get_selected():
            ...
            for g in iter_geom(geometric_data):
                if g is None:
                    continue
...
class AppExcEditor(QtCore.QObject):
    ...
    def connect_canvas_event_handlers(self):
        ...
        try:
            self.app.ui.slot.triggered.connect(self.exc_add_slot)
        except (TypeError, AttributeError):
            pass

        try:
            self.app.ui.slot_array.triggered.connect(self.exc_add_slot_array)
        except (TypeError, AttributeError):
            pass
            
        try:
            self.app.ui.drill_resize.triggered.connect(self.exc_resize_drills)
        except (TypeError, AttributeError):
            pass
    ...
    def disconnect_canvas_event_handlers(self):
        ...
        try:
            self.app.ui.slot.triggered.disconnect(self.exc_add_slot)
        except (TypeError, AttributeError):
            pass

        try:
            self.app.ui.slot_array.triggered.disconnect(self.exc_add_slot_array)
        except (TypeError, AttributeError):
            pass
        
        try:
            self.app.ui.drill_resize.triggered.disconnect(self.exc_resize_drills)
        except (TypeError, AttributeError):
            pass
        ...
        for storage_tooldia in self.storage_dict:
            for x in self.storage_dict[storage_tooldia].get_objects():
                if isinstance(x.geo, MultiLineString):
                    ...
                    first_linestring = next(iter(x.geo.geoms))
    ...
    def add_exc_shape(self, shape, storage):
        ...
        if not isinstance(shape, DrawToolShape):
            log.debug("AppExcEditor.add_exc_shape() --> Expected DrawToolShape, got %s" % str(type(shape)))
            return

        if shape.geo is None:
            log.debug("AppExcEditor.add_exc_shape() --> Empty geometry skipped: geo is None.")
            return

        if isinstance(shape.geo, list) and len(shape.geo) == 0:
            log.debug("AppExcEditor.add_exc_shape() --> Empty geometry skipped: geo is [].")
            return

        if hasattr(shape.geo, "is_empty") and shape.geo.is_empty:
            log.debug("AppExcEditor.add_exc_shape() --> Empty Shapely geometry skipped.")
            return
    ...
    def add_shape(self, shape):
        ...
        if not isinstance(shape, DrawToolShape):
            log.debug("AppExcEditor.add_shape() --> Expected DrawToolShape, got %s" % str(type(shape)))
            return

        if shape.geo is None:
            log.debug("AppExcEditor.add_shape() --> Empty geometry skipped: geo is None.")
            return

        if isinstance(shape.geo, list) and len(shape.geo) == 0:
            log.debug("AppExcEditor.add_shape() --> Empty geometry skipped: geo is [].")
            return

        if hasattr(shape.geo, "is_empty") and shape.geo.is_empty:
            log.debug("AppExcEditor.add_shape() --> Empty Shapely geometry skipped.")
            return
    ...
    def draw_utility_geometry(self, geo):
        ...
        if geo is None or geo.geo is None:
            return

        geo_data = geo.geo

        # Shapely 2.x compatibility: MultiLineString must use .geoms
        if isinstance(geo_data, MultiLineString):
            for linestring in geo_data.geoms:
                self.tool_shape.add(
                    shape=linestring,
        ...
        # Shapely 2.x compatibility: MultiPolygon must use .geoms
        if isinstance(geo_data, MultiPolygon):
            for poly in geo_data.geoms:
                self.tool_shape.add(
                    shape=poly,
                    color=(self.app.defaults["global_draw_color"] + '80'),
                    update=False,
        ...
        if isinstance(geo_data, GeometryCollection):
            for geo in geo_data.geoms:
                self.tool_shape.add(
                    shape=geo,
                    color=(self.app.defaults["global_draw_color"] + '80'),
                    update=False,
        ...
        # Normal list/tuple of geometries
        if isinstance(geo_data, (list, tuple)):
            for el in geo_data:
                if isinstance(el, MultiPolygon):
                ...
                elif isinstance(el, MultiLineString):
        ...
        # Single geometry: Polygon, LineString, LinearRing, etc.
        self.tool_shape.add(
            shape=geo_data,
            color=(self.app.defaults["global_draw_color"] + '80'),
            update=False,
    ...
    def plot_shape(self, geometry=None, color='0x000000FF', linewidth=1):
        ...
        if isinstance(geometry, DrawToolShape):
            return self.plot_shape(geometry=geometry.geo, color=color, linewidth=linewidth)

        # Shapely 2.x compatibility: multipart geometries must use .geoms
        if isinstance(geometry, MultiLineString):
            for line in geometry.geoms:
                plot_elements += self.plot_shape(geometry=line, color=color, linewidth=linewidth)
            return plot_elements
            
        if isinstance(geometry, MultiPolygon):
            for poly in geometry.geoms:
                plot_elements += self.plot_shape(geometry=poly, color=color, linewidth=linewidth)
            return plot_elements            
        ...
        # Polygon: descend into exterior and interiors
        if isinstance(geometry, Polygon):
            plot_elements += self.plot_shape(geometry=geometry.exterior, color=color, linewidth=linewidth)
            for interior in geometry.interiors:
                plot_elements += self.plot_shape(geometry=interior, color=color, linewidth=linewidth)
            return plot_elements
        ...
        # Point: Excellon editor does not plot points directly here
        if isinstance(geometry, Point):
            return plot_elements

        # Lists / tuples of geometries
        try:
            for geo in geometry:
                plot_elements += self.plot_shape(geometry=geo, color=color, linewidth=linewidth)
        except TypeError:
