...
class PdfParser(QtCore.QObject):
    ...
    def iter_geom(geometry):
        ...
        if isinstance(geometry, (list, tuple)):
            geo_list = []
            for geo in geometry:
                geo_list += PdfParser.iter_geom(geo)
            return geo_list
    ...
    @staticmethod
    def append_solid(storage, geometry):
        for poly in PdfParser.iter_geom(geometry):
    ...
    @staticmethod
    def append_clear(storage, geometry):
        for poly in PdfParser.iter_geom(geometry):
            if poly is None or poly.is_empty:
                continue
            if not isinstance(poly, Polygon):
        ...
        for pline in lines:
                ...
                if apertures_dict:
                    ...
                    if found_aperture:  
                        for pdf_geo in path_geo:
                            self.append_solid(apertures_dict[copy(found_aperture)]['geometry'], pdf_geo)
                    else:
                        ...
                        for pdf_geo in path_geo:
                            self.append_solid(apertures_dict[str(aperture)]['geometry'], pdf_geo)
                else:
                    apertures_dict[str(aperture)] = {}
                    apertures_dict[str(aperture)]['size'] = round(applied_size, 5)
                    apertures_dict[str(aperture)]['type'] = 'C'
            ...
            if match:
                ...
                if flag_clear_geo is True:
                    ...
                    try:
                        for pdf_geo in path_geo:
                            self.append_solid(apertures_dict['0']['geometry'], pdf_geo)
                ...
                if flag_clear_geo is True:
                    try:
                        for pdf_geo in fill_geo:
                            self.append_clear(apertures_dict['0']['geometry'], pdf_geo)
                    except KeyError:
                        # in case there is no stroke width yet therefore no aperture
                        apertures_dict['0'] = {
                            'size': round(applied_size, 5),
