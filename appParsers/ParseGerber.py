            ...
            if current_polarity == 'D':
                ...
                if self.app.defaults['gerber_extra_buffering']:
                    ...
                    if hasattr(self.solid_geometry, "geoms"):
                        for p in self.solid_geometry.geoms:
    ...
    def bounds(self, flatten=None):
        ...
        def bounds_rec(obj):
            ...
            if isinstance(obj, dict):
                for key in obj:
                    bx0, by0, bx1, by1 = bounds_rec(obj[key])
                    minx = min(minx, bx0)
                    miny = min(miny, by0)
                    maxx = max(maxx, bx1)
                    maxy = max(maxy, by1)
            ...
            if isinstance(obj, (list, tuple)):
                for geo in obj:
                    bx0, by0, bx1, by1 = bounds_rec(geo)
                    minx = min(minx, bx0)
                    miny = min(miny, by0)
                    maxx = max(maxx, bx1)
                    maxy = max(maxy, by1)
            ...
            try:
                ...
                log.debug("parseGerber.Gerber.bounds() --> %s" % str(e))
                return minx, miny, maxx, maxy

        bounds_coords = bounds_rec(self.solid_geometry)

        if bounds_coords[0] == np.Inf or bounds_coords[2] == -np.Inf:
            return 0, 0, 0, 0
