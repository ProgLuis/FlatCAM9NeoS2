...
class FCDoubleSlider(QtWidgets.QSlider):
    # from here: https://stackoverflow.com/questions/42820380/use-float-for-qslider
    ...
    def _to_slider_int(self, value):
        """
        Convert a real value to the internal integer value required by QSlider.
        QSlider only accepts integers.
        """
        return int(round(float(value) * self._multi))
    ...
    def setMinimum(self, value):
        return super(FCDoubleSlider, self).setMinimum(self._to_slider_int(value))

    def setMaximum(self, value):
        return super(FCDoubleSlider, self).setMaximum(self._to_slider_int(value))

    def setSingleStep(self, value):
        return super(FCDoubleSlider, self).setSingleStep(self._to_slider_int(value))

    def singleStep(self):
        return float(super(FCDoubleSlider, self).singleStep()) / self._multi

    def set_value(self, value):
        super(FCDoubleSlider, self).setValue(self._to_slider_int(value))
    ...
    def set_range(self, min, max):
        self.blockSignals(True)
        self.setRange(self._to_slider_int(min), self._to_slider_int(max))
        self.blockSignals(False)
...
class FCSliderWithDoubleSpinner(QtWidgets.QFrame):
    ...
    def __init__(self, min=0, max=10000.0000, step=1, precision=4, orientation='horizontal', **kwargs):
        ...
        self.slider = FCDoubleSlider(decimals=precision, orientation=orientation)
        self.slider.setMinimum(min)
        self.slider.setMaximum(max)
        self.slider.setSingleStep(step)
