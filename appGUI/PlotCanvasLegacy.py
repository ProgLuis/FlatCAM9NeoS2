...
class ShapeCollectionLegacy:
    ...
    def redraw(self, update_colors=None):
        ...
        path_num = 0        
        local_shapes = self._shapes.copy()
        ...
        try:
            ...
            log.debug("ShapeCollectionLegacy.redraw().patches.clear() --> %s" % str(e))
            ...
            if local_shapes[element]['visible'] is True:
                if obj_type == 'excellon':
                    ...
                    if self.obj.options["solid"] and isinstance(local_shapes[element]['shape'], Polygon):
                        try:
                            if local_shapes[element]['shape'] is None:
                                continue

                            if getattr(local_shapes[element]['shape'], 'is_empty', False):
                                continue
