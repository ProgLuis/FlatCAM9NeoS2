...
def _update_shape_buffers(data, triangulation='glu'):
    ...
    for geo_el in iter_geom(geo):
        if geo_el is None or getattr(geo_el, 'is_empty', False):
            continue

        try:
            simplified_geo = geo_el.simplify(tolerance) if tolerance else geo_el
        except Exception:
            simplified_geo = geo_el

        if simplified_geo is None or getattr(simplified_geo, 'is_empty', False):
            continue

        for simple_el in iter_geom(simplified_geo):
            pts = []
            tri_pts = []
            tri_tris = []

            if isinstance(simple_el, LineString):
                try:
                    pts = _linestring_to_segments(list(simple_el.coords))
                except Exception:
                    pts = []

            elif isinstance(simple_el, LinearRing):
                try:
                    pts = _linearring_to_segments(list(simple_el.coords))
                except Exception:
                    pts = []

            elif isinstance(simple_el, Polygon):
                if face_color is not None:
                    if triangulation == 'glu':
                        try:
                            gt = GLUTess()
                            tri_tris, tri_pts = gt.triangulate(simple_el)
                        except Exception as e:
                            print("VisPyVisuals._update_shape_buffers() --> triangulation failed: %s" % str(e))
                    else:
                        print("Triangulation type '%s' isn't implemented. Drawing only edges." % triangulation)

                if color is not None:
                    try:
                        pts = _linearring_to_segments(list(simple_el.exterior.coords))
                        for ints in simple_el.interiors:
