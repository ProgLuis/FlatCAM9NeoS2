...
class App(QtCore.QObject):
        ...
        class AboutDialog(QtWidgets.QDialog):
            def __init__(self, app, parent=None):
                ...
                title = QtWidgets.QLabel(
                    "<font size=8><B>FlatCAM 9 Neo S2</B></font><BR>"
                    "<font size=4>Shapely 2.x Friendly Edition</font><BR>"
                    "{title}<BR>"
                    "<BR>"
                    "<BR>"
                    "<a href = \"https://github.com/ProgLuis/FlatCAM9NeoS2\"><B>{devel}</B></a><BR>"
                    "<a href = \"https://github.com/ProgLuis/FlatCAM9NeoS2/releases\"><b>{down}</B></a><BR>".format(
                ...
                description_label = QtWidgets.QLabel(
                    "FlatCAM 9 Neo S2 ({date}) - {arch}<br>"
                    "Fork comunitario independiente<br>"
                    "<a href = \"https://github.com/ProgLuis/FlatCAM9NeoS2\">https://github.com/ProgLuis/FlatCAM9NeoS2</a><br>"
                    "Basado en FlatCAM original: <a href = \"http://flatcam.org/\">http://flatcam.org</a><br>".format(
                        date=version_date,
                ...
                self.prog_grid_lay.addWidget(QtWidgets.QLabel('%s' % "Luis Enrique Yacupoma Aguirre"), 5, 0)
                self.prog_grid_lay.addWidget(QtWidgets.QLabel('%s' % _("Neo S2 Maintainer >= 2026")), 5, 1)
                self.prog_grid_lay.addWidget(QtWidgets.QLabel('%s' % "<aulaendesarrollo@gmail.com>"), 5, 2)
                ...
                self.prog_grid_lay.addWidget(QtWidgets.QLabel(''), 6, 0)
        ...
        class HowtoDialog(QtWidgets.QDialog):
            def __init__(self, app, parent=None):
                ...
                content = QtWidgets.QLabel(
                    ...
                    (
                        _("This program is %s and free in a very wide meaning of the word.") % open_source_link,
                        _("Yet it cannot evolve without <b>community feedback</b>."),
                        _("If you would like to help improve FlatCAM 9 Neo S2,"),
                        _("you can contribute to the project by:"),
                        _("Reporting bugs or unexpected behavior."),
                        _("Sending suggestions or improvement ideas."),
                        _("Sending encouraging messages 😄"),
                        _("Main language: Spanish."),
                        _("English messages are also welcome (I can translate them)."),
                        _("GitHub: https://github.com/ProgLuis/FlatCAM9NeoS2"),
                        _("E-mail: aulaendesarrollo@gmail.com")
                    )
                )
    ...
    def on_delete(self, force_deletion=False):
        ...
        #Codigo para proteger el borrado sin seleccion de objetos desde el panel Project - INICIO
        selected_objects = self.collection.get_selected()

        if not selected_objects:
            self.inform.emit('[WARNING_NOTCL] %s' % _("No object is selected."))
            return
        #Codigo para proteger el borrado sin seleccion de objetos desde el panel Project - TERMINO
    ...
    def on_properties_tab_click(self, index):
        prop_widget = self.ui.properties_scroll_area.widget()
        if prop_widget is not None and prop_widget.objectName() == 'default_properties':
            self.setup_default_properties_tab()
            ...
            try:
                ...
                for tool_id, tool in tools.items():
                    ...
                    for drill in tool.get('drills', []):
                        flipped_drill = Point(drill.x, svg_height - drill.y)
                        flipped_tools[tool_id]['drills'].append(flipped_drill)
                        flipped_tools[tool_id]['solid_geometry'].append(
                            flipped_drill.buffer(tool['tooldia'] / 2.0)
                        )
