...
class ToolFiducials(AppTool):
    ...
    def iter_geom(geometry):
        ...
        if isinstance(geometry, (list, tuple)):
            geo_list = []
            for geo in geometry:
                geo_list += ToolFiducials.iter_geom(geo)
            return geo_list
    ...
    def add_fiducials_geo(self, points_list, g_obj, fid_size=None, fid_type=None, line_size=None):
        ...
        if fid_type == 'circular':
            ...
            follow_geo_list = [geo.centroid for geo in geo_list]
            ...
            if g_obj.solid_geometry:
                s_list += self.iter_geom(g_obj.solid_geometry)
            ...
            """Agregado"""
            follow_geo_list = self.iter_geom(geo_list)            
            ...
            geo_buff_list = unary_union(self.iter_geom(geo_buff_list))
            geo_buff_list = geo_buff_list.buffer(0)

            for poly in self.iter_geom(geo_buff_list):
        ...
        def initialize(grb_obj, app_obj):
            ...
            try:
                log.debug("ToolFiducials.initialize() --> start")
                log.debug("ToolFiducials.initialize() --> s_list len: %s" % len(s_list))
                log.debug("ToolFiducials.initialize() --> geo_list len: %s" % len(geo_list))

                grb_obj.solid_geometry = unary_union(self.iter_geom(s_list))
                log.debug("ToolFiducials.initialize() --> solid_geometry created: %s" % type(grb_obj.solid_geometry))


                base_follow_geometry = self.iter_geom(deepcopy(g_obj.follow_geometry)) if g_obj.follow_geometry else []
                log.debug("ToolFiducials.initialize() --> base_follow_geometry len: %s" % len(base_follow_geometry))
                
                grb_obj.follow_geometry = base_follow_geometry + self.iter_geom(follow_geo_list)
                
                
                log.debug("ToolFiducials.initialize() --> follow_geometry created")
                ...
                log.debug("ToolFiducials.initialize() --> export_gerber finished")

            except Exception as e:
                log.debug("ToolFiducials.initialize() --> FAILED: %s" % str(e))
                raise
    ...
    def flatten(self, geometry):
        ...
        """
        self.flat_geometry += self.iter_geom(geometry)
