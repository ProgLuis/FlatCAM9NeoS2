...
class SolderPaste(AppTool):
    ...
    def iter_geom(geometry):
        ...
        if isinstance(geometry, (list, tuple)):
            geo_list = []
            for geo in geometry:
                geo_list += SolderPaste.iter_geom(geo)
            return geo_list
    ...
    def on_create_geo(self, name, work_object, use_thread=True):
        ...
        def flatten(geometry=None, reset=True, pathonly=False):
            ...
            for geo in self.iter_geom(geometry):
                if geo is None or getattr(geo, 'is_empty', False):
                    continue

                if pathonly and isinstance(geo, Polygon):
            ...
            for tool in sorted_tools:
                ...
                for g in self.iter_geom(work_geo):
                    ...
                    try:
                        ...
                        log.debug("ToolSolderPaste.on_create_geo() diagonal intersection --> %s" % str(e))
                        rest_geo.append(g)
                        continue

                    if diag_1_intersect.is_empty or diag_2_intersect.is_empty:
                        rest_geo.append(g)
                        continue
                    ...
                    try:
                        ...
                        log.debug("ToolSolderPaste.on_create_geo() buffer(-offset) --> %s" % str(e))
                        rest_geo.append(g)
                        continue

                    if offseted_poly.is_empty:
                        rest_geo.append(g)
                        continue
                    ...
                    try:
                        ...
                        log.debug("ToolSolderPaste.on_create_geo() geo intersection offseted_poly --> %s" % str(e))
                        rest_geo.append(g)
                        continue
                ...
                work_geo = self.iter_geom(rest_geo)
                rest_geo[:] = []
    ...
    def on_create_gcode(self, name, workobject, use_thread=True):
        ...
        def job_init(job_obj, app_obj):
            ...
            for tooluid_key, tooluid_value in obj.tools.items():
                ...
                if not tooluid_value.get('solid_geometry'):
                    log.debug(
                        "ToolSolderPaste.on_create_gcode() --> skipping empty tool: %s"
                        % str(tooluid_key)
                    )
                    continue
                ...
                parsed_geo = []
                for geo in tool_cnc_dict['gcode_parsed']:
                    if 'geom' in geo:
                        parsed_geo += self.iter_geom(geo['geom'])

                if parsed_geo:
                    tool_cnc_dict['solid_geometry'] = unary_union(parsed_geo)
                else:
                    tool_cnc_dict['solid_geometry'] = []
            ...
            job_obj.source_file = StringIO(total_gcode)
