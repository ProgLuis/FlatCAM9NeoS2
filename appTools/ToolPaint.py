...
def iter_geom_recursive(obj):
    """
    Recursively yield non-empty geometries from nested lists, tuples,
    multipart geometries and GeometryCollections.
    ...
    if isinstance(obj, (list, tuple)):
        for item in obj:
            for sub_item in iter_geom_recursive(item):
                yield sub_item
        return

    if hasattr(obj, "geoms"):
        for item in obj.geoms:
            for sub_item in iter_geom_recursive(item):
                yield sub_item
        return
...
def iter_polygon_parts(obj):
    """
    Yield valid Polygon parts only.
    LinearRing is converted to Polygon.
    """
    for geo in iter_geom_recursive(obj):
        if isinstance(geo, LinearRing):
            geo = Polygon(geo)

        if isinstance(geo, Polygon) and not geo.is_empty and geo.is_valid:
            yield geo
...
class ToolPaint(AppTool, Gerber):
    ...
    def on_paint_button_click(self):
        ...
        self.laser_lines_warning_emitted = False
    ...
    def paint_polygon_worker(self, polyg, tooldiameter, paint_method, over, conn, cont, prog_plot, obj):
        ...
        if paint_method == 0:   # _("Standard")
            ...
            try:                
                ...
                if not traces_el_dict:
                    self.app.inform.emit(
                        '[WARNING_NOTCL] %s' %
                        _("No native trace/follow geometry found.\n"
                          "The Gerber may contain only exported polygon regions "
                          "(PDF/SVG converted Gerber).\n"
                          "Laser_lines works best with native PCB Gerber files.")
                    )
                    return None
                ...
                for aperture_size in traces_el_dict:
                    for elem in traces_el_dict[aperture_size]:
                        ...
                        if line:
                            ...
                            if t_o is None:
                                if not hasattr(self, 'laser_lines_warning_emitted'):
                                    self.laser_lines_warning_emitted = False

                                if self.laser_lines_warning_emitted is False:
                                    self.app.inform.emit(
                                        '[WARNING_NOTCL] %s' %
                                        _("No native trace/follow geometry found.\n"
                                          "The Gerber may contain only exported polygon regions "
                                    ...
                                    self.laser_lines_warning_emitted = True
                                continue
                ...
                for lin in iter_geom_recursive(lines_union):
                    if lin is not None and not lin.is_empty:
                        cpoly.insert(lin)
    ...
    def paint_geo(self, obj, geometry, tooldia=None, order=None, method=None, outname=None,
        ...
        def job_normal_clear(geo_obj, app_obj):
            ...
            for tool_dia in sorted_tools:
                ...
                poly_buf = []
                for pol in iter_polygon_parts(geometry):
                    buffered_pol = pol.buffer(-paint_offset)
                    if buffered_pol and not buffered_pol.is_empty:
                        for buffered_part in iter_polygon_parts(buffered_pol):
                            poly_buf.append(buffered_part)
        ...
        def job_rest_clear(geo_obj, app_obj):
            ...
            for pol in iter_polygon_parts(geometry):
                buffered_pol = pol.buffer(-paint_offset)
                if buffered_pol and not buffered_pol.is_empty:
                    for x in iter_polygon_parts(buffered_pol):
                        poly_buf.append(x)
            ...
            # variables to display the percentage of work done            
            poly_buf_parts = list(iter_polygon_parts(poly_buf))
            geo_len = len(poly_buf_parts)
            ...
            for tool_dia in sorted_tools:
                    ...
                    try:                        
                        for pp in poly_buf_parts:
                            ...
                            if rest and not rest.is_empty:
                                rest_list += list(iter_polygon_parts(rest))
                        ...
                        if rest and not rest.is_empty:
                            try:
                                ...
                                for r in iter_polygon_parts(rest):
                                    rest_list.append(r)
                ...
                tmp = list(iter_polygon_parts(poly_buf))
                tmp += rest_list
        ...
        if obj.kind == 'gerber':
            ...
            if self.app.defaults["tools_paint_plotting"] == 'progressive':
                if isinstance(obj.solid_geometry, list):
                    obj.solid_geometry = MultiPolygon(list(iter_polygon_parts(obj.solid_geometry))).buffer(0)
                else:
    ...
    def paint_poly_all(self, obj, tooldia=None, order=None, method=None, outname=None, tools_storage=None, plot=True,
        ...
        def recurse(geometry, reset=True):
            """
            Creates a list of valid Polygon objects.
            ...
            for geo in iter_geom_recursive(geometry):
                if self.app.abort_flag:
                    raise grace
    ...
    def paint_poly_area(self, obj, sel_obj, tooldia=None, order=None, method=None, outname=None,
        ...
        target_geo = unary_union(list(iter_polygon_parts(obj.solid_geometry)))
        if obj.kind == 'gerber':
            # I don't do anything here, like buffering when the Gerber is loaded without buffering????!!!!
            if self.app.defaults["gerber_buffering"] == 'no':
                msg = '%s %s %s' % (_("Paint Tool."),
    ...
    def paint_poly_ref(self, obj, sel_obj, tooldia=None, order=None, method=None, outname=None,
        ...
        try:
            geo_parts = list(iter_polygon_parts(geo))

            if not geo_parts:
                self.app.inform.emit('[ERROR_NOTCL] %s' % _("No object available."))
                return

            env_obj = unary_union(geo_parts).convex_hull
    ...
    def paint_bounds(geometry):
        geos = list(iter_geom_recursive(geometry))
        ...
        for geo in geos:
            try:
                minx_, miny_, maxx_, maxy_ = geo.bounds
            except Exception as e:
