...
class QRCode(AppTool):
    ...
    def make(self, pos):
        ...
        for geo in self.iter_geo(working_geo):
        ...
        for geo in self.iter_geo(self.qrcode_geometry):
            geo_elem = {
        ...
        # Force Gerber object refresh after QR geometry insertion
        try:
            self.grb_object.plot()
        except Exception as e:
            log.debug("QRCode.make() plot error --> %s" % str(e))

        try:
            self.app.plotcanvas.redraw()
        except Exception as e:
            log.debug("QRCode.make() redraw error --> %s" % str(e))

        QtCore.QTimer.singleShot(300, self.app.plotcanvas.redraw)
        QtCore.QTimer.singleShot(800, self.app.plotcanvas.redraw)
        QtCore.QTimer.singleShot(1500, self.app.plotcanvas.redraw)
    ...
    def draw_utility_geo(self, pos):
        ...
        qr_geo_count = len(list(self.iter_geo(self.qrcode_geometry)))

        if qr_geo_count <= self.app.defaults["tools_qrcode_sel_limit"]:
            for poly in self.iter_geo(self.qrcode_utility_geometry):
