...
from camlib import grace, iter_geom
...
def iter_polygon_parts(obj):
    """
    Yield valid Polygon parts from Polygon, MultiPolygon,
    GeometryCollection, list or tuple.
    Compatible with Shapely 1.x and 2.x.
    """
    for geo in iter_geom(obj):
        if geo is None:
            continue

        if hasattr(geo, "is_empty") and geo.is_empty:
            continue
...
def iter_line_parts(obj):
    """
    Yield LineString or LinearRing parts from LineString, MultiLineString,
    GeometryCollection, list or tuple.
    Compatible with Shapely 1.x and 2.x.
    """
    for geo in iter_geom(obj):
...
def geometry_is_empty(obj):
    """
    Safely check if a geometry container has no usable geometry.
    """
    if obj is None:
        return True
...
class ToolIsolation(AppTool, Gerber):
    ...
    def find_optim_mp(aperture_storage, decimals):
        ...
        total_geo = list(iter_polygon_parts(total_geo))

        if not total_geo:
            msg = '[ERROR_NOTCL] %s' % _("The Gerber object has no valid polygon geometry.")
            return msg, None

        total_geo = MultiPolygon(total_geo).buffer(0)
        total_geo_list = list(iter_polygon_parts(total_geo))

        geo_len = len(total_geo_list)
        ...
        min_dict = {}
        idx = 1
        for geo in total_geo_list:
            for s_geo in total_geo_list[idx:]:
        ...
        if not min_list:
            msg = '[ERROR_NOTCL] %s' % _("No distances between geometry elements could be found.")
            return msg, None
        ...
        def job_thread(app_obj):
            with self.app.proc_container.new(_("Checking ...")):
                try:
                    ...
                    if not total_geo:
                        app_obj.inform.emit('[ERROR_NOTCL] %s' % _("The Gerber object has no valid polygon geometry."))
                        return 'fail'
                    ...
                    if not min_list:
                        app_obj.inform.emit('[ERROR_NOTCL] %s' % _("No distances between geometry elements could be found."))
                        return 'fail'
        ...
        def buffer_task(app_obj):
            with app_obj.proc_container.new('%s...' % _("Buffering")):
                if isinstance(self.grb_obj.solid_geometry, list):
                    poly_parts = list(iter_polygon_parts(self.grb_obj.solid_geometry))
                    self.grb_obj.solid_geometry = MultiPolygon(poly_parts) if poly_parts else []

                if not self.grb_obj.solid_geometry:
                    app_obj.inform.emit('[ERROR_NOTCL] %s' % _("No valid geometry to buffer."))
                    return
    ...
    def isolate_handler(self, isolated_obj):
        ...
        if selection == 0:  # ALL
            ...
            ref_geo = unary_union(list(iter_geom(ref_obj.solid_geometry)))
            use_geo = unary_union(list(iter_geom(isolated_obj.solid_geometry))).difference(ref_geo)
        ...
        if combine:
            ...
            for tool in sorted_tools:
                ...
                for i in range(passes):
                    ...
                    new_solid_geo = [geo for geo in iter_geom(iso_geo) if geo is not None and not geo.is_empty]
    ...
    def combined_rest(self, iso_obj, iso2geo, tools_storage, lim_area, negative_dia=None, plot=True):
        ...
        for sorted_tool in sorted_tools:
            for tool in tools_storage:
                if float('%.*f' % (self.decimals, tools_storage[tool]['tooldia'])) == sorted_tool:
                    ...
                    if solid_geo == 'fail' or work_geo == 'fail':
                        self.app.inform.emit('[ERROR_NOTCL] %s' % _("Isolation geometry could not be generated."))
                        return 'fail'
                    ...
                    # make sure that no empty geometry element is in the solid_geometry                    
                    new_solid_geo = [geo for geo in iter_geom(solid_geo) if geo is not None and not geo.is_empty]
        ...
        if not total_solid_geometry:
            self.app.inform.emit('[ERROR_NOTCL] %s' % _("No isolation geometry was generated."))
            return 'fail'

        self.app.app_obj.new_object("geometry", iso_name, iso_init, plot=plot)

        # the tools are finished but the isolation is not finished therefore it failed
        if not geometry_is_empty(work_geo):
            self.app.inform.emit("[WARNING] %s" % _(
                "Partial failure. The geometry was processed with all selected tools.\n"
                "However, some geometry elements could not be isolated.\n"
                "Depending on the clearances in the Gerber design, more than one smaller tool may be required "
                "to complete the isolation process."
            ))

            msg = _(
                "Isolation could not be completed for all copper features with the selected tools. "
                "Review the following coordinates and consider adding smaller or more suitable tools:"
            )
            self.app.inform_shell.emit(msg)

            msg = ''
            for geo in iter_polygon_parts(work_geo):
    ...
    def combined_normal(self, iso_obj, iso2geo, tools_storage, lim_area, negative_dia=None, plot=True):
        ...
        for tool in sorted_tools:
            ...
            if not to_follow:
                work_geo_parts = list(iter_polygon_parts(work_geo))
                if not work_geo_parts:
                    self.app.inform.emit('[ERROR_NOTCL] %s' % _("No valid Gerber geometry to isolate."))
                    continue
                work_geo = unary_union(work_geo_parts)
            ...
            for nr_pass in range(passes):
                ...
                # Shapely 2.x safe path:
                # avoid Gerber.isolation_geometry() because it may recurse internally.
                if iso2geo is None and not to_follow:
                    try:
                        buffered_geo = work_geo.buffer(iso_offset)

                        if mill_dir:
                            reversed_polys = []
                            for p in iter_polygon_parts(buffered_geo):
                                reversed_polys.append(Polygon(p.exterior.coords[::-1], p.interiors))
                            buffered_geo = MultiPolygon(reversed_polys) if reversed_polys else []

                        if iso_t == 0:      # exterior
                            for p in iter_polygon_parts(buffered_geo):
                                solid_geo.append(p.exterior)
                        elif iso_t == 1:    # interiors
                            for p in iter_polygon_parts(buffered_geo):
                                for interior in p.interiors:
                                    solid_geo.append(interior)
                        else:               # full
                            for p in iter_polygon_parts(buffered_geo):
                                solid_geo.append(p.exterior)
                                for interior in p.interiors:
                        ...
                        log.debug('ToolIsolation.combined_normal() area safe path --> %s' % str(e))
                        self.app.inform_shell.emit('ToolIsolation.combined_normal() area safe path --> %s' % str(e))
                        self.app.inform.emit('[ERROR_NOTCL] %s' % _("Isolation geometry could not be generated."))
                        continue
                else:
                    envelope_geo = None if iso2geo is None and not to_follow else work_geo

                    iso_geo = self.generate_envelope(iso_offset, mill_dir, geometry=envelope_geo, env_iso_type=iso_t,
    ...
    def area_subtraction(self, geo, subtraction_geo=None):
        """
        Subtracts the subtraction_geo from geo.
        Compatible with Shapely 1.x and 2.x.
        """
        new_geometry = []

        if subtraction_geo:
            sub_union = unary_union(list(iter_geom(subtraction_geo)))
        else:
            name = self.ui.exc_obj_combo.currentText()
            subtractor_obj = self.app.collection.get_by_name(name)
            sub_union = unary_union(list(iter_geom(subtractor_obj.solid_geometry)))

        for geo_elem in iter_geom(geo):
            if geo_elem is None or geo_elem.is_empty:
                continue
            ...
            if isinstance(geo_elem, Polygon):
                for ring in self.poly2rings(geo_elem):
                    ...
                    if new_geo and not new_geo.is_empty:
                        for part in iter_geom(new_geo):
                            if part is not None and not part.is_empty:
                                new_geometry.append(part)
                ...
                for line_elem in iter_line_parts(geo_elem):
    ...
    def area_intersection(self, geo, intersection_geo=None):
        ...
        intersect_union = unary_union(list(iter_geom(intersection_geo)))
        ...
        for geo_elem in iter_geom(geo):
            ...
            if isinstance(geo_elem, Polygon):
                for ring in self.poly2rings(geo_elem):
                    new_geo = LineString(ring).intersection(intersect_union)
                    if new_geo and not new_geo.is_empty:
                        for part in iter_geom(new_geo):
                            if part is not None and not part.is_empty:
                                new_geometry.append(part)
    ...
    def on_poly_mouse_click_release(self, event):
        ...
        if event.button == 1:
            ...
            if self.poly_dict:
                ...
                # Refresco diferido del canvas.
                # En Polygon Selection, a veces el Gerber original deja una huella visual
                # al ocultarse; esto fuerza repintado sin afectar la geometría generada.
                QtCore.QTimer.singleShot(100, self.app.plotcanvas.redraw)
                QtCore.QTimer.singleShot(300, self.app.plotcanvas.redraw)                
            else:
    ...
    def selection_area_handler(self, start_pos, end_pos, sel_type):
        ...
        for geo in iter_polygon_parts(self.solid_geometry):
            if geo in self.poly_dict.values():
                continue

            if sel_type is True:
                add_geo = geo.within(poly_selection)
            else:
                add_geo = poly_selection.intersects(geo)
    ...
    def clear_area_selection_shapes(self):
        """
        Clean Area Selection visual helper shapes after isolation.
        Used with QTimer because some canvas redraws happen after isolate().
        """
        try:
            self.delete_tool_selection_shape()
        except Exception as e:
            log.debug("ToolIsolation.clear_area_selection_shapes() delete_tool_selection_shape --> %s" % str(e))

        try:
            self.delete_moving_selection_shape()
        except Exception as e:
            log.debug("ToolIsolation.clear_area_selection_shapes() delete_moving_selection_shape --> %s" % str(e))

        try:
            self.app.delete_selection_shape()
        except Exception as e:
            log.debug("ToolIsolation.clear_area_selection_shapes() app.delete_selection_shape --> %s" % str(e))

        try:
            self.app.tool_shapes.clear(update=True)
        except Exception as e:
            log.debug("ToolIsolation.clear_area_selection_shapes() tool_shapes.clear --> %s" % str(e))
    ...
    def on_mouse_release(self, event):
        ...
        if event.button == 1:
            ...
            selected_geo = []
            for geo in iter_polygon_parts(self.grb_obj.solid_geometry):
                if geo.intersects(self.sel_rect):
                    selected_geo.append(geo)

            if not selected_geo:
                self.app.inform.emit('[ERROR_NOTCL] %s' % _("No Gerber geometry found inside the selected area."))
                self.delete_tool_selection_shape()
                self.app.delete_selection_shape()
                self.sel_rect = []
                return
            
            #La doble llamada es intencional: una limpia rápido, y la segunda limpia si el canvas redibuja la sombra un poquito después.            
            self.isolate(
                isolated_obj=self.grb_obj,
                geometry=None,
                limited_area=self.sel_rect,
                plot=True
            )

            self.sel_rect = []

            QtCore.QTimer.singleShot(100, self.clear_area_selection_shapes)
            QtCore.QTimer.singleShot(300, self.clear_area_selection_shapes)
    ...
    def generate_envelope(self, offset, invert, geometry=None, env_iso_type=2, follow=None, nr_passes=0,
        ...
        if follow:
            ...
            try:
                ...
                self.app.inform_shell.emit('ToolIsolation.generate_envelope() --> %s' % str(e))
                return 'fail'
    ...
    def generate_rest_geometry(geometry, tooldia, passes, overlap, invert, env_iso_type=2, negative_dia=None,
        ...
        geometry_parts = list(iter_polygon_parts(geometry))

        for idx, geo in enumerate(geometry_parts):
            good_pass_iso = []
            start_idx = idx + 1
            ...
            for nr_pass in range(passes):
                ...
                for geo_search_idx in range(idx):                    
                    if check_geo.intersects(geometry_parts[geo_search_idx]):
                        intersect_flag = True
                        break

                
                if intersect_flag is False:
                    for geo_search_idx in range(start_idx, len(geometry_parts)):
                        if check_geo.intersects(geometry_parts[geo_search_idx]):
                            intersect_flag = True
                            break
                        ...
                        if geo.interiors:
                            ...
                            if len_interiors > 1:
                                ...
                                if isinstance(temp_geo, Polygon):
                                    ...
                                    temp_geo_len = len(list(iter_geom(temp_geo)))
                                    if total_poly_len != temp_geo_len:
                                        # some interiors could not be isolated
                                        break
    ...
    def find_polygon_ignore_interiors(self, point, geoset=None):
        """
        Find a polygon that contains the point while ignoring interiors.
        Compatible with Shapely 1.x and 2.x.
        """
        if geoset is None:
        ...
        for sub_geo in iter_polygon_parts(geoset):
            try:
                poly_ext = Polygon(sub_geo.exterior)
                if poly_ext.contains(pt):
                    return sub_geo
            except AttributeError:
                continue
