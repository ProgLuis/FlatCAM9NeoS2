...
class Properties(AppTool):
    ...
    def iter_geom(geometry):
        ...
        if isinstance(geometry, (list, tuple)):
            geo_list = []
            for geo in geometry:
                geo_list += Properties.iter_geom(geo)
            return geo_list
    ...
    def addItems(self, obj):
        ...
        def job_thread(obj_prop):
            ...
            log.debug("PropertiesTool --> kind: %s" % obj_prop.kind)
            log.debug("PropertiesTool --> solid_geometry type: %s" % type(geo))
            try:
                log.debug("PropertiesTool --> solid_geometry elements: %d" % len(geo))
            except Exception:
                log.debug("PropertiesTool --> solid_geometry has no len()")
            ...
            if geo:
                ...
                if obj_prop.kind.lower() == 'gerber':
                    ...
                    geo_list = self.iter_geom(geo)

                    try:

                        if len(geo_list) > 20:
                            log.warning(
                                "PropertiesTool --> skipping copper area; geometry too complex."
                            )
                            copper_area = 0

                        else:
                            for geo_el in geo_list:
                        ...
                        log.warning(
                            "PropertiesTool --> copper area failed: %s" % str(e)
                        )
                        copper_area = 0                    
            ...
            if obj_prop.kind.lower() != 'cncjob':
                ...
                if geo:
                    ...
                    if geo_list:
                        try:
                            if len(geo_list) > 20:
                                log.warning("PropertiesTool --> skipping convex hull; geometry too complex.")
                                area_chull = 0
                            else:
                                env_obj = unary_union(geo_list)
                                env_obj = env_obj.convex_hull
                                area_chull = env_obj.area
                        except Exception as e:
                            log.warning("PropertiesTool --> convex hull failed: %s" % str(e))
                            area_chull = 0
                    else:
                        area_chull = 0                     
