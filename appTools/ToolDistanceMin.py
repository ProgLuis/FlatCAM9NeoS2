...
def make_safe_geometry(geometry):
    """
    Build a safe Shapely geometry from Polygon, MultiPolygon,
    GeometryCollection, list or tuple.
    Compatible with Shapely 1.x and 2.x.
    """
    geo_parts = [
        geo for geo in iter_geom(geometry)
    ...
    def activate_measure_tool(self):
        ...
        if self.app.call_source == 'app':
            ...
            if len(selected_objs) != 2:
                ...
                first_geo = make_safe_geometry(selected_objs[0].solid_geometry)
                last_geo = make_safe_geometry(selected_objs[1].solid_geometry)

                if first_geo is None or last_geo is None:
                    self.app.inform.emit('[WARNING_NOTCL] %s' % _("One or both selected objects have no valid geometry."))
                    return
