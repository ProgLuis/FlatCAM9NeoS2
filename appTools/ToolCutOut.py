...
from shapely.geometry import box, MultiPolygon, Polygon, LineString, LinearRing, MultiLineString, GeometryCollection
...
class CutOut(AppTool):
    ...
    def on_freeform_cutout(self):
        ...
        def cutout_handler(geom, gapsize):
            ...
            for g in CutOut.iter_geom(geom):
        ...
        with self.app.proc_container.new("Generating Cutout ..."):
            ...
            def job_thread(app_obj):
                ...
                if kind == 'single':
                    ...
                    object_geo = CutOut.iter_geom(object_geo)
                ...
                if has_mouse_bites is True:
                    ...
                    if kind == 'single':
                        ...
                        mb_object_geo = CutOut.iter_geom(mb_object_geo)
                    ...
                    holes = []
                    for line in CutOut.iter_line_parts(mouse_bites_geo):
                        calc_len = 0
    ...
    def on_manual_geo(self):
        ...
        def geo_init(geo_obj, app_obj):
            ...
            if convex_box:
                ...
                geo_union_parts = CutOut.iter_geom(geo_union)
                if isinstance(geo_union, Polygon) or \
                        (isinstance(geo_union, list) and len(geo_union) == 1) or \
                        (isinstance(geo_union, MultiPolygon) and len(geo_union_parts) == 1):
                    base_geo = geo_union_parts[0] if isinstance(geo_union, MultiPolygon) else geo_union
                    geo_obj.solid_geometry = base_geo.buffer(margin + abs(dia / 2)).exterior
                ...
                if isinstance(geo, Polygon):
                    ...
                    solid_geo = []
                    for poly in CutOut.iter_geom(geo):
            ...
            if self.ui.gaptype_radio.get_value() == 'mb':
                with self.app.proc_container.new("Generating Excellon ..."):
                    ...
                    def job_thread(app_obj):
                        ...
                        mb_dia = self.ui.mb_dia_entry.get_value()
                        mb_spacing = self.ui.mb_spacing_entry.get_value()
                        for line in CutOut.iter_line_parts(self.mb_manual_cuts):
                            calc_len = 0
                            while calc_len < line.length:
                                holes.append(line.interpolate(calc_len))
                                calc_len += mb_dia + mb_spacing
    ...
    def on_mouse_move(self, event):
        ...
        def get_angle(geo):
            ...
            line = CutOut.longest_line_part(line)
        ...
        try:
            rot_angle = 0
            for geo_el in CutOut.iter_geom(man_geo):
    ...
    def iter_geom(geometry):
        """
        Return a safe list of geometry parts.
        Shapely 2.x multipart geometries are no longer directly iterable,
        therefore we must use the .geoms attribute when present.
        """
        if geometry is None:
            return []
        ...
        if isinstance(geometry, (list, tuple, set)):
            result = []
            for geo in geometry:
                result.extend(CutOut.iter_geom(geo))
            return result
    ...
    def iter_line_parts(geometry):
        """
        Return only line-like parts from simple, multipart or collection geometries.
        This is useful for mouse bites and manual gap operations where interpolate()
        must be applied to line paths, not polygons or geometry collections.
        """
        result = []
        for geo in CutOut.iter_geom(geometry):
            if geo is None or geo.is_empty:
                continue
            if isinstance(geo, Polygon):
                if not geo.exterior.is_empty:
                    result.append(geo.exterior)
                result.extend([ring for ring in geo.interiors if not ring.is_empty])
            elif isinstance(geo, (LineString, LinearRing)):
                result.append(geo)
        return result

    @staticmethod
    def longest_line_part(geometry):
        """
        Return the longest line-like component from a geometry result.
        Intersection can return LineString, MultiLineString, GeometryCollection
        or even points; only line-like pieces are useful for angle calculation.
        """
        lines = CutOut.iter_line_parts(geometry)
        if not lines:
            return None
    ...
    def flatten(geometry):
        ...
        Polygons are expanded into their exterior and interiors.
        ...
        for geo in CutOut.iter_geom(geometry):
            ...
            if isinstance(geo, Polygon):
                ...
                flat_geo.extend([ring for ring in geo.interiors if not ring.is_empty])
            else:
                flat_geo.append(geo)
        return flat_geo
    ...
    def recursive_bounds(geometry):
        ...
        for obj in CutOut.iter_geom(geometry):
            if obj is None or obj.is_empty:
                continue
            minx_, miny_, maxx_, maxy_ = obj.bounds
            minx = min(minx, minx_)
            miny = min(miny, miny_)
            maxx = max(maxx, maxx_)
            maxy = max(maxy, maxy_)
    ...
    def subtract_geo(target_geo, subtractor):
        ...
        for target in flat_geometry:
            if isinstance(target, (LineString, LinearRing, MultiLineString)):
                diff = target.difference(toolgeo)
    ...
    def intersect_geo(target_geo, second_geo):
        ...
        results = []
        for geo in CutOut.iter_geom(target_geo):
            if geo and not geo.is_empty and second_geo.intersects(geo):
                inter_geo = second_geo.intersection(geo)
                if inter_geo and not inter_geo.is_empty:
                    results.append(inter_geo)
