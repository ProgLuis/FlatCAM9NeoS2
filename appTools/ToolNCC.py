...
def make_multipolygon(obj):
    """
    Build a MultiPolygon from valid polygon parts only.
    Returns an empty MultiPolygon if no valid polygons are found.
    """
    poly_parts = list(iter_polygon_parts(obj))
    return MultiPolygon(poly_parts) if poly_parts else MultiPolygon()
...
class NonCopperClear(AppTool, Gerber):
    ...
    def calculate_bounding_box(self, ncc_obj, ncc_select, box_obj=None):
        ...
        if ncc_select == 0:     # _('Itself')
            geo_n = list(iter_geom(ncc_obj.solid_geometry))
            try:
                if not geo_n:
                    self.app.inform.emit('[ERROR_NOTCL] %s' % _("No object available."))
                    return None, None
                env_obj = unary_union(geo_n).convex_hull  
            ...
            env_obj = unary_union(list(iter_geom(self.sel_rect)))
            env_obj = list(iter_geom(env_obj))
            ...
            if box_kind == 'geometry':                      
                env_obj = list(iter_geom(box_geo))            

            elif box_kind == 'gerber':
                box_geo = unary_union(list(iter_geom(box_obj.solid_geometry))).convex_hull
                ncc_geo = unary_union(list(iter_geom(ncc_obj.solid_geometry))).convex_hull
    ...
    def apply_margin_to_bounding_box(self, bbox, box_kind, ncc_select, ncc_margin):
        ...
        if ncc_select == 0:     # _('Itself')
            ...
            geo_buff_list = []            
            for poly in iter_polygon_parts(bbox):
                if self.app.abort_flag:
                    # graceful abort requested by the user
                    raise grace
                geo_buff_list.append(poly.buffer(distance=ncc_margin, join_style=base.JOIN_STYLE.mitre))            
            new_bounding_box = unary_union(geo_buff_list) if geo_buff_list else MultiPolygon()
        elif ncc_select == 2:   # _("Reference Object")
            if box_kind == 'geometry':
                geo_buff_list = []                
                for poly in iter_polygon_parts(bbox):
    ...
    def clip_isolation_geometry(self, isolated_geo, bounding_box):
        """
        Clip isolation geometry to bounding_box.
        Compatible with Shapely 1.x and 2.x.
        """
        new_geometry = []

        for geo_elem in iter_geom(isolated_geo):
        ...
        if ncc_obj.kind == 'gerber' and not isotooldia:
            ...
            if has_offset is True:
                ...
                if isinstance(sol_geo, list):
                    sol_geo = make_multipolygon(sol_geo)
            ...
            for tool_iso in isotooldia:
                ...
                new_geometry = self.clip_isolation_geometry(isolated_geo, bounding_box)
            ...
            sol_geo = unary_union(list(iter_geom(isolated_geo)))
            if has_offset is True:
                self.app.inform.emit('[WARNING_NOTCL] %s ...' % _("Buffering"))
                sol_geo = sol_geo.buffer(distance=ncc_offset)
                self.app.inform.emit('[success] %s ...' % _("Buffering finished"))
            ...
            sol_geo = unary_union(list(iter_geom(ncc_obj.solid_geometry)))
    ...
    def clear_copper(self, ncc_obj, ncctooldia, isotooldia, sel_obj=None, outname=None, order=None,
                    ...
                    if len(area.geoms) > 0:
                        ...
                        for p in area.geoms:
                            ...
                            if p is not None and p.is_valid:
                                ...
                                for pol in iter_polygon_parts(p):
                                    QtWidgets.QApplication.processEvents()
        ...
        def gen_clear_area_rest(geo_obj, app_obj):
            ...
            while sorted_clear_tools:
                ...
                if area.geoms and len(area.geoms) > 0:
                    ...
                    for p in area.geoms:
                        ...
                        if p is not None and p.is_valid and not p.is_empty:
                            ...
                            if check_buff is None or check_buff.is_empty:
                                continue    
                ...
                new_area = [pol for pol in iter_polygon_parts(area) if pol.is_valid and not pol.is_empty]
                area = MultiPolygon(new_area) if new_area else MultiPolygon()
    ...
    def clear_copper_tcl(self, ncc_obj, sel_obj=None, ncctooldia=None, isotooldia=None, margin=None, has_offset=None,
        ...
        if ncc_select == 0:     # itself
            geo_n = list(iter_geom(ncc_sel_obj.solid_geometry))
            ...
            geo_n = unary_union(list(iter_geom(self.sel_rect)))

            geo_buff_list = []
            for poly in iter_polygon_parts(geo_n):
                if self.app.abort_flag:
                    raise grace
                geo_buff_list.append(poly.buffer(distance=ncc_margin, join_style=base.JOIN_STYLE.mitre))

            bounding_box = unary_union(geo_buff_list) if geo_buff_list else MultiPolygon()
            ...
            if ncc_sel_obj.kind == 'geometry':
                ...
                geo_n = unary_union(list(iter_geom(geo_n))).convex_hull
                ncc_geo = unary_union(list(iter_geom(self.ncc_obj.solid_geometry))).convex_hull
                bounding_box = ncc_geo.intersection(geo_n)
        ...
        def gen_clear_area(geo_obj, app_obj):
            ...
            if ncc_obj.kind == 'gerber' and not isotooldia:
                ...
                if self.app.defaults['gerber_buffering'] == 'no':
                    ...
                    sol_geo = ncc_obj.solid_geometry
                    if isinstance(sol_geo, list):
                        sol_geo = unary_union(list(iter_geom(sol_geo)))
                        ...
                        for p in area.geoms:
                            ...
                            if p is not None and p.is_valid:
                                ...
                                if isinstance(p, Polygon):
                                    ...
                                    for poly_p in iter_polygon_parts(p):
    ...
    def get_ncc_empty_area(self, target, boundary=None):
        ...
        target_parts = list(iter_geom(target))
        geo_len = len(target_parts)

        if isinstance(target, list):
            target = make_multipolygon(target)
