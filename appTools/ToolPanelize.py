...
class Panelize(AppTool):
    ...
    def iter_geom(geometry):
        ...
        if isinstance(geometry, (list, tuple)):
            geo_list = []
            for geo in geometry:
                geo_list += Panelize.iter_geom(geo)
            return geo_list
            ...
            if panel_source_obj is not None:
                ...
                def job_init_geometry(obj_fin, app_obj):
                    ...
                    def translate_recursion(geom):
                        ...
                        for local_geom in self.iter_geom(geom):
                            try:
                                translated.append(affinity.translate(local_geom, xoff=currentx, yoff=currenty))
                            except Exception as e:
                                log.debug("Panelize.translate_recursion() --> %s" % str(e))
                    ...
                    if panel_source_obj.kind == 'geometry':
                        if panel_source_obj.multigeo is True:
                            for tool in panel_source_obj.tools:
                                try:                                    
                                    geo_len += len(self.iter_geom(panel_source_obj.tools[tool]['solid_geometry']))
                                except TypeError:
                                    geo_len += 1
                    elif panel_source_obj.kind == 'gerber':
                            ...
                            if panel_source_obj.kind == 'geometry':
                                if panel_source_obj.multigeo is True:
                                    for tool in panel_source_obj.tools:
                                        ...
                                        # panelization
                                        pol_nr = 0                                        
                                        for geo_el in self.iter_geom(panel_source_obj.tools[tool]['solid_geometry']):
                                    ...
                                    # calculate the number of polygons
                                    try:                                        
                                        geo_len = len(self.iter_geom(panel_source_obj.solid_geometry))
                                    except TypeError:
                                        geo_len = 1
                                    ...
                                    pol_nr = 0
                                    try:                                        
                                        for geo_el in self.iter_geom(panel_source_obj.solid_geometry):
                                            if app_obj.abort_flag:
                                                # graceful abort requested by the user
                                                raise grace
                        ...
                        for tool in obj_fin.tools:
                            ...
                            for geo in obj_fin.tools[tool]['solid_geometry']:
                                if isinstance(geo, LineString):
                                    ...
                                    for line in self.iter_geom(geo):
                                        lines.append(line)
                                else:
                            ...
                            if lines:
                                ...
                                fused_lines = [unary_union(self.iter_geom(fused_lines))]
                            else:
                                fused_lines = []
