...
class AlignObjects(AppTool):
    ...
    def add_clicked_point(self, point):
        self.clicked_points.append(
            [
                float('%.*f' % (self.decimals, point.x)),
                float('%.*f' % (self.decimals, point.y))
            ]
        )
        self.check_points()
    ...
    def on_mouse_click_release(self, event):
        ...
        if event.button == 1:
            ...
            if self.app.selection_type is not None:
                ...
                # Free Point mode:
                # accept the raw clicked coordinate without requiring follow geometry.
                if self.align_type == 'fp':
                    self.add_clicked_point(click_pt)
                    return
                    ...
                    for tool, tool_dict in self.target_obj.tools.items():
                        for geo in tool_dict['solid_geometry']:
                            if click_pt.within(geo):
                                ...
                                self.add_clicked_point(center_pt)
                                point_found = True
                                return
                ...
                if point_found is False:
                    self.app.inform.emit(
                        '[WARNING_NOTCL] %s' %
                        _("No valid pad/drill point detected. Try 'Free Point' mode for polygon-only Gerber files.")
                    )
    ...
    def check_points(self):
        ...
        if len(self.clicked_points) == 2:
            if self.align_type in ['sp', 'fp']:
                self.align_translate()
                self.app.inform.emit('[success] %s' % _("Done."))
                self.app.plot_all()
                
                # Limpa la barra de estado
                QtCore.QTimer.singleShot(500, self.app.proc_container.view.set_idle)
                QtCore.QTimer.singleShot(1500, self.app.proc_container.view.set_idle)
                QtCore.QTimer.singleShot(3000, self.app.proc_container.view.set_idle)
