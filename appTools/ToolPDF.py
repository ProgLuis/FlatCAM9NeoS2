...
class ToolPDF(AppTool):
    ...
    def iter_geom(geometry):
        ...
        if isinstance(geometry, (list, tuple)):
            geo_list = []
            for geo in geometry:
                geo_list += ToolPDF.iter_geom(geo)
            return geo_list
    ...
    def open_pdf(self, filename):
        ...
        log.debug("ToolPDF.open_pdf() --> started for: %s" % short_name)
        log.debug("ToolPDF.open_pdf() --> promises: %s" % str(self.parsing_promises))
        ...
        with self.app.proc_container.new(_("Parsing ...")):
            with open(filename, "rb") as f:
                ...
                # ---------------------------------------------------------
                # FlatCAM PDF parser supports RGB vector PDFs only.
                # CMYK PDFs may generate invalid geometry, slow rendering
                # or parser instability.
                # ---------------------------------------------------------
                if re.search(rb'(\s|^)([0-9.]+\s+){4}[kK](\s|$)', pdf):

                    log.debug(
                        "ToolPDF.open_pdf() --> CMYK PDF detected. "
                        "FlatCAM supports RGB vector PDFs only."
                    )

                    self.app.inform.emit(
                        '[ERROR_NOTCL] %s' %
                        _("CMYK PDF detected. FlatCAM currently supports RGB vector PDFs only.")
                    )
            ...
            for stream_header, s in re.findall(self.stream_re, pdf):
                ...
                try:
                    ...
                    if b'FlateDecode' in stream_header:
                        try:
                            raw_stream = zlib.decompress(s)
                        except Exception as dec_err:
                            log.debug("ToolPDF.open_pdf() --> skipped bad Flate stream %d: %s" % (stream_nr, str(dec_err)))
                            continue
                    else:
                        raw_stream = s
                    ...
                    if not any(op in decoded_stream for op in pdf_ops):
                        log.debug("ToolPDF.open_pdf() --> skipped non-vector/binary stream: %d" % stream_nr)
                        continue

                    self.pdf_decompressed[short_name] += decoded_stream + '\r\n'

                except Exception as e:
                    log.debug("ToolPDF.open_pdf() --> skipped stream %d: %s" % (stream_nr, str(e)))
                    continue

            log.debug("ToolPDF.open_pdf() --> decompressed size: %d" % len(self.pdf_decompressed[short_name]))
            log.debug("ToolPDF.open_pdf() --> starting parse_pdf()")
            
            if not self.pdf_decompressed[short_name].strip():
                log.debug("ToolPDF.open_pdf() --> no vector PDF content found for: %s" % short_name)
                self.app.inform.emit('[WARNING_NOTCL] %s' % _("No vector PDF content found. This PDF may be raster/image based."))
                self.parsing_promises.remove(short_name)
                return
    
            vector_ops = [' m', ' l', ' c', ' re', ' S', ' s', ' f', ' F', ' B', ' b']
            if not any(op in self.pdf_decompressed[short_name] for op in vector_ops):
                log.debug("ToolPDF.open_pdf() --> no usable vector operators found for: %s" % short_name)
                self.app.inform.emit('[WARNING_NOTCL] %s' % _("No usable vector geometry found. This PDF may be raster/image based."))
                self.parsing_promises.remove(short_name)
                return
            

            parsed_pdf = self.parser.parse_pdf(
                pdf_content=self.pdf_decompressed[short_name]
            )
            if not parsed_pdf:
                log.debug("PDF parsing returned empty structure.")
                self.app.inform.emit(
                    '[WARNING_NOTCL] %s' %
                    _("PDF contains no usable vector geometry.")
                )
                self.parsing_promises.remove(short_name)
                return
            ...
            if has_geometry is False:
                log.debug("PDF contains only empty layers.")
                self.app.inform.emit(
                    '[WARNING_NOTCL] %s' %
                    _("PDF has no drawable vector data.")
                )
                self.parsing_promises.remove(short_name)
                return
            ...
            log.debug("ToolPDF.open_pdf() --> parse_pdf() finished")
