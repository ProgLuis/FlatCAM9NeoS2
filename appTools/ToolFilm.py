...
class Film(AppTool):
    ...
    def save_png_from_svg_qt(self, svg_text, filename, dpi=96):
        try:
            svg_bytes = QtCore.QByteArray(svg_text.encode('utf-8'))
            renderer = QtSvg.QSvgRenderer(svg_bytes)

            if not renderer.isValid():
                log.debug("FilmTool.save_png_from_svg_qt() --> Invalid SVG renderer.")
                return False

            default_size = renderer.defaultSize()
            width = max(1, int(default_size.width() * dpi / 96))
            height = max(1, int(default_size.height() * dpi / 96))
            ...
            log.debug("FilmTool.save_png_from_svg_qt() --> %s" % str(e))
            return False
    ...
    def generate_positive_punched_film(self, name, boxname, source, factor, ftype='svg'):
        ...
        if source == 'exc':
            ...
            exc_geo = self.iter_geom(exc_obj.solid_geometry)
            film_geo = self.iter_geom(film_obj.solid_geometry)

            if not exc_geo or not film_geo:
                self.app.inform.emit('[ERROR_NOTCL] %s' % _("Failed. Empty geometry."))
                return 'fail'

            exc_solid_geometry = unary_union(exc_geo)
            film_solid_geometry = unary_union(film_geo)

            punched_solid_geometry = film_solid_geometry.difference(exc_solid_geometry)
            ...
            if not punching_geo:
                self.app.inform.emit('[ERROR_NOTCL] %s' % _("Failed. No punch geometry."))
                return 'fail'

            punching_geo = unary_union(self.iter_geom(punching_geo))

            film_geo = self.iter_geom(film_obj.solid_geometry)
            if not film_geo:
                self.app.inform.emit('[ERROR_NOTCL] %s' % _("Failed. Empty film geometry."))
                return 'fail'

            temp_solid_geometry = unary_union(film_geo)
    ...
    def export_negative(self, obj_name, box_name, filename, boundary,
        ...
        def make_negative_film(scale_factor_x, scale_factor_y):
            ...
            exported_svg = ET.tostring(root, encoding='unicode')
            ...
            if ftype == 'svg':
                ...
                try:
                    if self.save_png_from_svg_qt(doc_final, filename, new_png_dpi) is False:
                        return 'fail'
            ...
            if ftype == 'svg':
                ...
                try:
                    ...
                    if p_size == 'Bounds':
                        ...
                        #renderPDF.draw(drawing, my_canvas, 0, 0)
                        #margin = 10 * mm
                        #renderPDF.draw(drawing, my_canvas, margin, margin)                        
                        page_width, page_height = page_size
                        draw_x = (page_width - drawing.width) / 2
                        draw_y = (page_height - drawing.height) / 2
                        renderPDF.draw(drawing, my_canvas, draw_x, draw_y)
                        my_canvas.save()
                except Exception as e:
