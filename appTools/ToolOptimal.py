...
class ToolOptimal(AppTool):
    ...
    def iter_geom(geometry):
        ...
        if isinstance(geometry, (list, tuple)):
            geo_list = []
            for geo in geometry:
                geo_list += ToolOptimal.iter_geom(geo)
            return geo_list
    ...
    def find_minimum_distance(self):
        ...
        def job_thread(app_obj):
            ...
            try:
                    ...
                    if 'geometry' in fcobj.apertures[ap]:
                        ...
                        for geo_el in fcobj.apertures[ap]['geometry']:
                            ...
                            if 'solid' in geo_el and geo_el['solid'] is not None:
                                for solid_geo in self.iter_geom(geo_el['solid']):
                                    if solid_geo is not None and not solid_geo.is_empty and solid_geo.is_valid:
                                        total_geo.append(solid_geo)
                ...
                total_geo = self.iter_geom(total_geo)
                ...
                if len(total_geo) > 2000:
                    app_obj.inform.emit('[ERROR_NOTCL] %s' %
                                        _("The Gerber object is too complex for Optimal Tool analysis. "
                                          "Please simplify the geometry first."))
                    log.warning("ToolOptimal --> geometry too complex: %d elements." % len(total_geo))
                    return 'fail'

                geo_len = (len(total_geo) * (len(total_geo) - 1)) / 2
