...
def iter_valid_geometries(geometry):
    """
    Yield non-empty Shapely geometries from Polygon, MultiPolygon,
...
class ToolEtchCompensation(AppTool):
    ...
    def set_tool_ui(self):
        ...
        self.ui.factor_entry.set_value(3.0)
        self.ui.offset_entry.set_value(0.0)
    ...
    def on_compensate(self):
        ...
        if ratio_type == 'factor':
            ...
            if factor_value is None or factor_value == 0:
                self.app.inform.emit('[WARNING_NOTCL] %s' %
                                     _("Please enter a valid non-zero value for Etch Factor."))
                return
            ...
            if offset_value is None:
                self.app.inform.emit('[WARNING_NOTCL] %s' %
                                     _("Please enter a valid value for Offset."))
                return
        ...
        for poly in iter_valid_geometries(grb_obj.solid_geometry):
            new_solid_geometry.append(poly.buffer(offset, int(grb_circle_steps)))

        if not new_solid_geometry:
            self.app.inform.emit('[WARNING_NOTCL] %s' % _("No valid geometry found for etch compensation."))
            return
            ...
            for k in new_apertures[ap]:
                ...
                if k == 'geometry':
                    for geo_el in new_apertures[ap][k]:
                        if 'solid' in geo_el:
                            solid_parts = [
                                geo.buffer(offset, int(grb_circle_steps))
                                for geo in iter_valid_geometries(geo_el['solid'])
                            ]

                            if solid_parts:
                                geo_el['solid'] = unary_union(solid_parts)
