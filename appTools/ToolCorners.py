...
class ToolCorners(AppTool):
    ...
    def add_corners_geo(self, points_storage, g_obj):
        ...
        s_list = list(iter_valid_geometries(g_obj.solid_geometry))

        geo_buff_list = [
            geo for geo in iter_valid_geometries(geo_buff_list)
        ]

        if geo_buff_list:
            geo_buff_list = unary_union(geo_buff_list).buffer(0)
            s_list.extend(list(iter_valid_geometries(geo_buff_list)))
        ...
        def initialize(grb_obj, app_obj):
            ...
            grb_obj.solid_geometry = unary_union(s_list) if s_list else []
