...
class ToolSub(AppTool):
    ...
    def iter_geom(geometry):
        ...
        if isinstance(geometry, (list, tuple)):
            geo_list = []
            for geo in geometry:
                geo_list += ToolSub.iter_geom(geo)
            return geo_list
    ...
    def new_gerber_object(self, outname, output):
        ...
        def obj_init(grb_obj, app_obj):
            ...
            try:
                ...
                for geo_el in self.iter_geom(poly_buff):
                    if geo_el is not None and not geo_el.is_empty:
                        if isinstance(geo_el, Polygon):
                            clean_poly_buff.append(geo_el)

                if clean_poly_buff:
                    poly_buff = unary_union(clean_poly_buff)

                    try:
                        poly_buff = poly_buff.buffer(0.0000001)
                    except Exception as e:
                        log.debug("ToolSub.new_gerber_object() positive buffer failed --> %s" % str(e))

                    try:
                        poly_buff = poly_buff.buffer(-0.0000001)
                    except Exception as e:
                        log.debug("ToolSub.new_gerber_object() negative buffer failed --> %s" % str(e))
                else:
                    poly_buff = []

            except Exception as e:
                log.debug("ToolSub.new_gerber_object() solid_geometry rebuild failed --> %s" % str(e))
                poly_buff = []
    ...
    def on_subtract_geo_click(self):
        ...
        if self.sub_geo_obj.multigeo:
            ...
            for tool in self.sub_geo_obj.tools:
                if 'solid_geometry' in self.sub_geo_obj.tools[tool]:
                    sub_geo_list += self.iter_geom(
                        self.sub_geo_obj.tools[tool]['solid_geometry']
                    )

            if not sub_geo_list:
                self.app.inform.emit('[ERROR_NOTCL] %s' %
                                     _("Subtractor geometry is empty."))
                return

            self.sub_union = unary_union(sub_geo_list)
        else:
            self.sub_union = unary_union(self.iter_geom(self.sub_geo_obj.solid_geometry))
    ...
    def toolgeo_intersection(self, tool, geo):
        ...
        with self.app.proc_container.new(text):
            ...
            if self.ui.close_paths_cb.isChecked():                
                new_geo = unary_union(self.iter_geom(geo)).difference(self.sub_union)
                if new_geo:
                    if not new_geo.is_empty:
            ...
            # resulting paths are unclosed resulting in a multitude of rings
                ...
                for geo_elem in self.iter_geom(geo):
    ...
    def periodic_check_handler(self):
        ...
        try:
            if not self.promises:
                ...
                # reset the type of subtraction for next time
                self.sub_type = None
        except Exception as e:
