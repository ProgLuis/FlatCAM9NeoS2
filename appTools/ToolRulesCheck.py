...
class RulesCheck(AppTool):
    ...
    def iter_geom(geometry):
        ...
        if isinstance(geometry, (list, tuple)):
            geo_list = []
            for geo in geometry:
                geo_list += RulesCheck.iter_geom(geo)
            return geo_list
    ...
    def check_inside_gerber_clearance(gerber_obj, size, rule):
        ...
        if clear_geo:
            ...
            total_geo = RulesCheck.iter_geom(solid_geo)
            if not total_geo:
                obj_violations['points'] = ['Failed. Empty geometry.']
                return rule_title, [obj_violations]

            total_geo = unary_union(total_geo).buffer(0.000001)
            total_geo = RulesCheck.iter_geom(total_geo)
    ...
    def check_gerber_clearance(gerber_list, size, rule):
        ...
        total_geo_grb_1 = RulesCheck.iter_geom(total_geo_grb_1)
        total_geo_grb_3 = RulesCheck.iter_geom(total_geo_grb_3)

        if not total_geo_grb_1 or not total_geo_grb_3:
            obj_violations['name'] = []
            obj_violations['points'] = ['Failed. Empty geometry.']
            return rule_title, [obj_violations]

        total_geo_grb_1 = RulesCheck.iter_geom(unary_union(total_geo_grb_1).buffer(0))
        total_geo_grb_3 = RulesCheck.iter_geom(unary_union(total_geo_grb_3).buffer(0))
    ...
    def execute(self):
        ...
        def worker_job(app_obj):
            ...
            try:
                ...
                log.debug("RulesCheck.worker_job() set_idle --> %s" % str(e))
