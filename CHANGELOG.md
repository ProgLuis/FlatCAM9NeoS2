...
* Completed the PDF Compatibility Suite for FlatCAM 9 Neo S2.
* This milestone consolidates the third major CAD/CAM compatibility suite in the project:
  * SVG Compatibility Suite.
  * DXF Compatibility Suite.
  * PDF Compatibility Suite.
* The PDF workflow is now organized around vector PDF files intended for CNC/CAM preparation.
* The validated PDF compatibility described in this milestone applies to vector PDF content.
* Raster PDF handling remains experimental and is documented as a future improvement area.

PDF Workflow Completed:

* Added two independent PDF import workflows:
  * `Import -> PDF as Geometry Object`.
  * `Import -> PDF as Gerber Object`.
* The historical PDF import path is now identified conceptually as `PDF as Gerber Object`.
* This naming is consistent with the existing SVG and DXF import workflows.

PDF as Geometry Object workflow:
...
* Reorganized the historical PDF import code to represent the two PDF processing branches more clearly.
* `appTools/ToolPDF.py` is preserved as the historical compatibility wrapper.
* `appTools/ToolPDFGeometry.py` controls the `PDF as Geometry Object` workflow.
* `appTools/ToolPDFGerber.py` controls the `PDF as Gerber Object` workflow.
* `appParsers/PDFGeometryBuilder.py` builds Geometry objects from supported PDF vector/raster processing paths.
* `appParsers/PDFGerberBuilder.py` builds the parsed Gerber/Excellon PDF structures used by the PDF as Gerber workflow.
* `appParsers/PDFPreparedInput.py` prepares temporary selected-page or cropped PDF inputs for the Gerber workflow when needed.
* `appParsers/PDFContentAnalyzer.py` provides PDF content diagnostics.
* `appParsers/PDFSourceAdvisor.py` converts PDF analysis data into user-facing import guidance.
* `appTools/PDFImportAssistant.py` provides page, crop, preview and complexity guidance shared by the PDF workflows.
* This organization improves maintainability and allows both importers to evolve independently.

Menu and UI naming:

* The historical `PDF Import Tool` was renamed in the visible UI as `PDF as Gerber Object`.
* A separate `PDF as Geometry Object` workflow is available for Geometry import.
* User-facing messages and assistant guidance were aligned with the new naming.
* Internal class and file names were not renamed unnecessarily when compatibility would be safer.

PDF as Geometry Object internal flow:

* Vector PDF handling uses PyMuPDF drawing extraction and Shapely geometry construction.
* The Geometry workflow can create a temporary selected-page PDF when it is useful for multipage vector handling.
* Crop-aware vector processing can also operate directly from PyMuPDF drawing data.
* The raster pipeline uses a temporary SVG Tiny 1.2 bridge so the existing `ParseSVG.py` workflow can be reused.
* Original PDF files are never modified.
* Temporary files are removed automatically after successful processing.
* This architecture keeps the Geometry workflow open for future vector and raster improvements.

PDF as Gerber Object internal flow:

* The Gerber workflow uses temporary prepared PDF inputs when page selection or crop preparation is required.
* This is especially important for multipage PDF files.
* The temporary PDF is isolated from the original file.
* The original PDF file is never modified.
* Temporary prepared PDF files are removed after use.
* This keeps the historical PDF-to-Gerber parser stable while allowing safer page and crop handling.

Validated PDF compatibility:

* Validated with vector PDF files generated by:
  * Adobe Illustrator.
  * CorelDRAW.
  * Proteus.
* Validation applies to vector PDF content only.
* Raster-only PDF conversion remains under investigation and is not part of the validated PDF Compatibility Suite.
* Mixed vector/raster PDF files are diagnosed so the user can decide the safest workflow.

PDF Content Analyzer and guidance:

* Added PDF source/content analysis before import.
* The analyzer reports source hints, content type, page count and warnings.
* The PDF Source Advisor provides recommendations for vector, raster, mixed and unknown PDF content.
* The PDF Import Assistant provides page selection, preview, crop selection and vector complexity guidance.
* This helps the user choose between PDF as Geometry, PDF as Gerber or future raster workflows.

New PDF-related libraries:

* PyMuPDF (`fitz`):
  * Used for PDF opening, page access, preview rendering, page/crop handling, vector drawing extraction and temporary PDF preparation.
* Pillow (`PIL`):
  * Used in the raster pipeline to convert rendered PDF pages into image data suitable for binarization.
* `numpy`:
  * Used in the raster pipeline for image thresholding and binary bitmap preparation.
* `potracer` package / `potrace` Python module:
  * Used in the experimental raster pipeline to trace binary bitmap data.
* `svgwrite`:
  * Used to generate the temporary SVG Tiny 1.2 bridge consumed by the existing SVG parser.
* Existing Shapely geometry handling remains the Geometry construction foundation.

Future Improvements:

* Raster PDF:
  * Automatic raster PDF vectorization remains planned for future versions.
  * The intended research path is raster detection, binarization, cleanup, vectorization, SVG Tiny 1.2 bridge and Geometry generation.
  * This feature is still in investigation and design.
* Excel Detection:
  * Automatic detection of spreadsheet-like tables or Excel-style information inside PDF files is planned for future versions.
  * No table or spreadsheet extraction feature is implemented in this milestone.
...
FlatCAM 9 Neo S2 now has dedicated compatibility suites for the main interchange formats currently used in the project's CAD/CAM workflows. This improves import consistency and prepares the project for safer future expansion.
...
* Completed the `Import -> SVG as Gerber Object` workflow.
* Added full SVG Tiny 1.2 compatibility across both major SVG import paths.
* Improved support for files generated by Proteus PCB Design.
* Improved support for files generated by Adobe Illustrator.
* Added better interpretation of Compound Paths with partially overlapping contours.
* Improved handling of complex winding structures, including `nonzero` and `evenodd` fill rules.
* Added more reliable conversion of SVG artwork into Gerber geometry.
* Improved preservation of filled regions during SVG-to-Gerber conversion.
* Improved CAM workflow compatibility from SVG creation through Isolation Geometry and CNCJob generation.

Additional benefit:

* The Compound Path, overlap and winding interpretation improvements introduced for Gerber import also improve the previously completed `Import -> SVG as Geometry Object` workflow.

Result:

FlatCAM 9 Neo S2 now provides a complete SVG Tiny 1.2 workflow for both Geometry and Gerber import operations.

This milestone completes SVG Tiny 1.2 support across both major SVG import paths and significantly improves compatibility with common Proteus and Adobe Illustrator export pipelines.
...
This milestone specifically completed the `Import -> SVG as Geometry Object` workflow. Subsequent SVG parser improvements introduced on 2026-06-22 further enhanced geometry interpretation while completing the `Import -> SVG as Gerber Object` workflow.
